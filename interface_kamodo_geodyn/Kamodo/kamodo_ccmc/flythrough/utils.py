...
create_astropy(inTime, c1, c2, c3, inCoord, inType): Create as AstroPy SkyCoord 
    object and return.
spacepy_spacepy(spacepy_coord, outCoord, outType): Converts given SpacePy object 
    to the requested SpacePy coordinate system.
astropy_astropy(astropy_coord, outCoord, outType): Converts given Astropy object 
    to the requested AstroPy coordinate system.
spacepy_astropy(spacepy_coord, outCoord, outType): Converts given SpacePy object 
    to an AstroPy object in the requested coordinate system.
astropy_spacepy(astropy_coord, outCoord, outType): Converts given Astropy object 
    to a SpacePy object in the requested coordinate system.    
extract_spacepy(inTime, c1, c2, c3, inCoord, inType, outCoord, outType, newC, 
    verbose=False): Extract x, y, z, and units from a SpacePy Coords object and return.
...
ConvertCoord_f(inTime,c1,c2,c3,iC1,iT1,iC2,iT2): Wrapper function for ConvertCoord
    to be compatible with integer inputs for coordinate systems and types.
'''
...
astropy_coordlist = ['teme','icrs','fk5','fk4','itrs','galactic','cirs','tete',
                     'galactocentric','precessedgeocentric','geocentricmeanecliptic',
...
spacepy_coordlist = ['GDZ','GEO','GSM','GSE','SM','GEI','MAG','SPH','RLL']
...
#Steve Morley recommended converting to something other than a string to improve speed
@np.vectorize
...
def create_astropy(inTime, c1, c2, c3, inCoord, inType):
    ...
    if inType=='car':
        astropy_coord = SkyCoord(c1*u.Rearth, c2*u.Rearth, c3*u.Rearth, obstime=t, 
                             frame=inCoord, representation_type='cartesian')
    elif inType=='sph':
        astropy_coord = SkyCoord(c1*u.deg, c2*u.deg, c3*u.Rearth, obstime=t, 
                             frame=inCoord, representation_type='spherical')     
    return astropy_coord

def spacepy_spacepy(spacepy_coord, outCoord, outType):
    '''Converts given SpacePy object to the requested SpacePy coordinate system.'''
    
    new_spacepy_coord = spacepy_coord.convert(outCoord,outType)    #perform conversion
    return new_spacepy_coord

def astropy_astropy(astropy_coord, outCoord, outType):
    '''Converts given Astropy object to the requested AstroPy coordinate system.'''
    
    new_astropy_coord = astropy_coord.transform_to(outCoord)  #convert to requested coord frame
    if outType=='car': new_astropy_coord.representation_type='cartesian'
    elif outType=='sph': new_astropy_coord.representation_type='spherical'
    return new_astropy_coord

def spacepy_astropy(spacepy_coord, outCoord, outType):
    '''Converts given SpacePy object to an AstroPy object in the requested coordinate system.'''
...
def astropy_spacepy(astropy_coord, outCoord, outType):
    '''Converts given Astropy object to a SpacePy object in the requested coordinate system.'''
    
    cvals = Coords.from_skycoord(astropy_coord)  #always to GEO car
    spacepy_coord = spacepy_spacepy(cvals, outCoord, outType) #perform conversion in SpacePy
    return spacepy_coord
    
def extract_spacepy(inTime, c1, c2, c3, inCoord, inType, outCoord, outType, newC, verbose=False):
...
def extract_astropy(astropy_coord, outType):
    '''Extract x, y, z, and units from an AstroPy SkyCoord object and return.'''
    
    #retrieve converted values by the attribute names, converting to the standard 
    #  unit based on car vs sph
...
def ConvertCoord(inTime,c1,c2,c3,inCoord,inType,outCoord,outType,verbose=False):
    ...
    from one coordinate system to another. It will correct obvious errors in the 
    return units, but may not catch all incorrect values.
    ...
    inCoord string: case-sensitive string from list:
        'GDZ', 'GEO', 'GSM', 'GSE', 'SM', 'GEI', 'MAG', 'SPH', 'RLL' (SpacePy coordinates)
        'teme', 'icrs', 'fk5', 'fk4', 'itrs', 'galactic', 'galactocentric',
        'cirs', 'tete', 'precessedgeocentric', 'geocentricmeanecliptic', 
        'geocentrictrueecliptic', 'hcrs', 'barycentricmeanecliptic', 
        'heliocentricmeanecliptic', 'barycentrictrueecliptic', 'heliocentrictrueecliptic',
        'heliocentriceclipticiau76', 'custombarycentricecliptic', 'lsr', 'lsrk',
        'lsrd', 'supergalactic', 'galacticlsr', 'fk4noeterms' (AstroPy coordinates)
        Note: Not compatible with AstroPy's HADec and AltAz coordinate systems. 
        Note: Conversions using the galactocentric coordinate system are not conserved
            (a conversion to and then from this coordinate system does not return
             the same beginning values).                 
    ...
    *SpacePy's GDZ car coordinate system requires and produces (x, y, z) in km.
    ...
    The intepretations of the input coordinates vary between the AstroPy coordinate
        systems (see below). We leave it to the user to determine the proper
        input values accordingly.
    The longitude values returned for a given coordinate converted to an AstroPy 
        coordinate system are always positive (0 to 360 degrees).
    ...
    AstroPy coordinate systems: https://docs.astropy.org/en/stable/coordinates/skycoord.html
    Spherical coordinates in the AstroPy coordinate systems are interpreted as described 
    below with the units [x,y,z] = [deg, deg, R_E]. (All z values must be in R_E, not km.) 
    ...
    if (inCoord not in astropy_coordlist) and (inCoord not in spacepy_coordlist):
        raise ValueError(f'The {inCoord} coordinate system is not recogized. Must '+\
              f'be one of: {spacepy_coordlist} {astropy_coordlist}.')
    #print(inCoord, inType, outCoord, outType)
    ...
    ### ZACH EDIT
#     print('utils/ConvertCoord():  input = ',c1, c2,c3, sep='')
    ...
    if inCoord==outCoord and inType==outType:
        ...
        ### ZACH EDIT
#         print('utils/ConvertCoord():  output1 = ',c1, c2,c3, sep='')
        return c1, c2, c3, units

    #Create coordinate object
    if inCoord in spacepy_coordlist: coord_obj = create_spacepy(inTime, c1, c2, c3, inCoord, inType)
    elif inCoord in astropy_coordlist: coord_obj = create_astropy(inTime, c1, c2, c3, inCoord, inType)
    #print(coord_obj)
    ...
    ### ZACH EDIT
#     print('utils/ConvertCoord():  output2 = ',new_coord, sep='')
    ...
    if outCoord in spacepy_coordlist: 
        x, y, z, units = extract_spacepy(inTime, c1, c2, c3, inCoord, inType, 
                                         outCoord, outType, new_coord, verbose=verbose)
    elif outCoord in astropy_coordlist:
        x, y, z, units = extract_astropy(new_coord, outType)

    #Ending messages and final return.
    toc = time.perf_counter()
    if verbose: print('Converted from ',inCoord,inType,'to:',outCoord,outType,units,'in',"{:0.4f}".format(toc-tic),'seconds.')
    
    ### ZACH EDIT
#     print('utils/ConvertCoord():  output3 = ',x,y,z,units, sep='')
...
def ComputeLshell(inTime,c1,c2,c3,inCoord,inType):
    ...
    It uses the IRBEM library to do the calculations. (Not currently used in Kamodo.)
    ...
    *SpacePy's GDZ car coordinate system requires (x, y, z) in km.
    inCoordName:  string: case-sensitive string from list:
        'GDZ', 'GEO', 'GSM', 'GSE', 'SM', 'GEI', 'MAG', 'SPH', 'RLL' (SpacePy coordinates)
        'teme', 'icrs', 'fk5', 'fk4', 'itrs', 'galactic', 'galactocentric',
        'cirs', 'tete', 'precessedgeocentric', 'geocentricmeanecliptic', 
        'geocentrictrueecliptic', 'hcrs', 'barycentricmeanecliptic', 
    ...
    #compute Lstar, etc
    #Need to test what spacepy coord sys this works fastest in (GSM sph as above?)
...
#Not sure we need this. If so, will need to add integers for astropy coordinates.
