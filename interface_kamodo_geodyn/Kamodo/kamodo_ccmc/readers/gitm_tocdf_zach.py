...
gitm_varnames = {'Argon Mixing Ratio': ['r_Ar', 'linear', ''],
                 ...
                 'HeatFlux_Photoelectron': ['phi_qPhotoE', 'linear', 'W/m**2'],             # zach added
                 'HeatFlux_ChemicalHeating': ['phi_qChemicalHeating', 'linear', 'W/m**2'],  # zach added
                 'HeatFlux_RadCooling': ['phi_qRadCool', 'linear', 'W/m**2'],               # zach added
                 'HeatFlux_CO2Cooling': ['phi_qCO2Cooling', 'linear', 'W/m**2'],            # zach added
                 'HeatFlux_OCooling': ['phi_qOCooling', 'linear', 'W/m**2'],                # zach added
#                  'HeatFlux_***': ['phi_q***', 'linear', 'W/m**2'],                          # zach added
                    }
...
name_dict = {"Altitude": "Altitude", "Ar Mixing Ratio": "Argon Mixing Ratio",
             ...
             'AltIntPhotoElectronHeating (W/m2)': 'HeatFlux_Photoelectron',   # zach added
             'AltIntChamicalHeating (W/m2)': 'HeatFlux_ChemicalHeating',      # zach added, note the typo from GITM          
             'AltIntRadCooling (W/m2)':      'HeatFlux_RadCooling',           # zach added
             'AltIntCO2Cooling (W/m2)':      'HeatFlux_CO2Cooling',           # zach added
             'AltIntOCooling (W/m2)':        'HeatFlux_OCooling',             # zach added
#              'AltInt********** (W/m2)':     'HeatFlux_**********',            # zach added
            }    
...
unit_dict = {"Altitude": "m", "Ar Mixing Ratio": "", "Ar": "kg/m**3",
             ...
             'AltIntPhotoElectronHeating (W/m2)': 'W/m**2',   # zach added
             'AltIntChamicalHeating (W/m2)': 'W/m**2',        # zach added, note the typo from GITM      
             'AltIntRadCooling (W/m2)':   'W/m**2',           # zach added
             'AltIntCO2Cooling (W/m2)':   'W/m**2',           # zach added
             'AltIntOCooling (W/m2)':   'W/m**2',             # zach added
#              'AltInt********** (W/m2)':   'W/m**2',           # zach added
...
scale_dict = {"Altitude": "linear", "Ar Mixing Ratio": "linear",
              ...
              'AltIntPhotoElectronHeating (W/m2)' : 'linear',  # zach added
              'AltIntChamicalHeating (W/m2)': 'linear',        # zach added, note the typo "chamical" came from GITM       
              'AltIntRadCooling (W/m2)':   'linear',           # zach added
              'AltIntCO2Cooling (W/m2)':   'linear',           # zach added
              'AltIntOCooling (W/m2)':   'linear',             # zach added
#              'AltInt********** (W/m2)':   'linear',          # zach added
...
def _read(varlist, attrs, newfile=True):
    ...
    #### Zach added to make smaller files for the NEUTRALS ONLY
    neutrals_list = ['Longitude',
                    'Latitude',
                    'Altitude',
                    'Rho',                # mass density
#                     'O!D2!N',             # mass density of molecular oxygen
#                     'N!D2!N',             # mass density of molecular nitrogen
#                     'He',                   # mass density of helium
#                     'O(!U3!NP)',          # mass density of atomic oxygen  (3P state), 
                                          # ground state of atomic oxygen has two unpaired
                                          # electrons (bi‐radical), and is designated as 3P ("triplet P") state. 
                    'Temperature',

#                     'NO',               # mass density of nitric oxide
#                     'N(!U4!NS)',          # mass density of atomic nitrogen (4S state)
#                     'N(!U2!ND)',
#                     'N(!U2!NP)',
#                     'H',                  # mass density of hydrogen
#                     'CO!D2!N',
#                     'O(!U1!ND)',
#                     'V!Dn!N (east)',
#                     'V!Dn!N (north)',
...
#     f = open(attrs['file'], 'rb')
    with open(attrs['file'], 'rb') as f:   # zach changed so that the file auto closes after exiting.
...
#             if gvar:
            if gvar and v in neutrals_list:   # ZACH EDIT
#                 print('Zach-- _read neutrals: ', v)
...
def GitmBin(filename, flag_2D, verbose=False):
    ...
    varlist = []
    attrs = {'file': filename}
    print('Zach-- GitmBin',filename[-25:])
