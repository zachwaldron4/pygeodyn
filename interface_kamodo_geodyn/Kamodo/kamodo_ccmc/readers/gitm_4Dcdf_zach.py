...
def MODEL():
    ...
    class MODEL(Kamodo):
        ...
        def __init__(self, full_file_prefix, variables_requested=[],
                     filetime=False, verbose=True, gridded_int=True,
            ...
            if len(file_prefix.split('_')[-1]) == 2:  # asking for hourly files
                self.patterns = unique([basename(f)[:16] for f in total_files
                                        if     basename(f)[13:16] != '.nc'  and
                                            ...
                                            '.h'   not in basename(f)    and 
                                            'log'   not in basename(f)   and 
                                            'info'   not in basename(f)  and 
                                            '.txt'   not in basename(f)  and 
                                            'files'   not in basename(f) and 
                                            'list'   not in basename(f)
                                       ])
                day_tag = False
            ...
            print('# of files in self.patterns:  ', len(self.patterns))
            print('# of .nc files:               ', n_ncfiles)
            #############################################################
            #### CREATE THE .NC FIELS HERE if the number of .nc files matches the number of # of .bin files
            # if .nc files not there, create then
            self.conversion_test = True  # default value
            if len(self.patterns) != n_ncfiles:  # create .nc files for each pattern
                ...
                if sum(['2D' in p for p in self.patterns]) > 0:
                    ...
                    if verbose:
                        print('2D files found, will not calc 2D variables')
                else:
                    flag_2D = False  # calc will occur
                    if verbose:
                        print('2D files NOT found, 2D variables will be calculated (takes much longer if False)')
                if verbose:
                    print('Files names are:', self.patterns)
            ...
            if fulltime:  # add boundary time (default value)
                ...
                if current_idx+1 == len(file_prefixes):
                    ...
                    if not kamodo_test.conversion_test:
                        ...
                        if verbose:
                            print(f'Took {perf_counter()-t_search:.5f}s to find the best file.')
            ...
            for i in range(len(self.patterns)):
                ...
                setattr(self, '_time'+str(i), array(cdf_data.variables['time']))
                ...
                self.varfiles[self.patterns[i]] = [model_varnames[key][0] for key in gvar_list]
            ...
            if filecheck:
                ...
                for i in range(len(self.patterns)):
                    new_time = ts_to_hrs(short_data['time'], self.filedate)
                    setattr(self, '_time'+str(i), append(getattr(self, '_time'+str(i)),
                                                         new_time))
            else:
                self._time = t
