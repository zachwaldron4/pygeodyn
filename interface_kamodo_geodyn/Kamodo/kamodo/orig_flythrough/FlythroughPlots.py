...
def Plot4Dcar(variable_name, sat_time, sat_lon, sat_lat, sat_z, result, result_units, 
           plot_file, sat_zunits='km', plot_close=True, plot_sampling=4000):
...
def Plot4D_generic(variable_name, sat_time, c1, c2, c3, result, coord_type, coord_grid, 
                   result_units, plot_file, plot_close=True, plot_sampling=4000):
    '''make a 3D heat map of result, and a group of time series plots'''
    ...
    #get coordinate units and coordinate names
    coord_names = MW.coord_names(coord_type, coord_grid)
    coord_units = MW.coord_units(coord_type, coord_grid)
    if coord_grid=='sph':
    ...
    ax.yaxis.set_major_locator(MaxNLocator(5)) 
    ax.zaxis.set_major_locator(MaxNLocator(5)) 
    p = ax.scatter(c1[::i], c2[::i], c3[::i], result[::i], 
                   c=result[::i], cmap='viridis')  #works!
    ax.set_xlabel(coord_names['c1']+' ['+coord_units['c1']+']', labelpad=15)
    ax.set_ylabel(coord_names['c2']+' ['+coord_units['c2']+']', labelpad=15)
    ax.set_zlabel(coord_names['c3']+' ['+coord_units['c3']+']', labelpad=15)
    cbar = fig.colorbar(p, pad=0.1)  #add label and units to colorbar
    cbar.set_label('Interpolated '+variable_name+' ['+result_units+']', rotation=90, labelpad=25)
    try:  #remove pre-existing file if it exists
        os.remove(plot_file+variable_name+'_3Dheat.png')
    except:
        pass
    ...
    axs[0].set_ylabel('Interpolated \n'+variable_name+' \n['+result_units+']', labelpad=10)
    axs[0].set_xticks([])
    axs[1].plot(sat_time,c1, 'black')
    axs[1].set_ylabel(coord_names['c1'][:3]+'\n['+coord_units['c1']+']', labelpad=10)
    axs[1].set_xticks([])
    axs[2].plot(sat_time,c2, 'black')
    axs[2].set_ylabel(coord_names['c2'][:3]+'\n['+coord_units['c2']+']', labelpad=10)
    axs[2].set_xticks([])
    axs[3].xaxis.set_major_formatter(mdates.DateFormatter('%m/%d/%Y\n%H:%M:%S'))
    axs[3].xaxis.set_major_locator(MaxNLocator(5))     
    axs[3].plot([datetime.utcfromtimestamp(t) for t in sat_time], c3, 'black')
    axs[3].set_ylabel(coord_names['c3'][:3]+'\n['+coord_units['c3']+']')
    axs[3].set_xlabel('Satellite Time [UTC]', labelpad=10)   #add datetime for x axis
    try:  #remove pre-existing file if it exists
        os.remove(plot_file+variable_name+'_1Dlines.png')
    except:
