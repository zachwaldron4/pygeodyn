...
from numpy import vectorize, ones, pad, nan, hstack, vstack, array, expand_dims, linspace, floor, isnan, ndarray, integer, floating, generic, bool_, number, nanmax, nanmin
from numpy import float as np_float
from numpy import abs    as np_abs
from numpy.distutils.exec_command import exec_command
from pandas import DataFrame, Index, DatetimeIndex, Series, to_datetime
from pandas import MultiIndex
# from pandas import datetime as pd_datetime   # ZACH MOVED THIS TO BELOW to where it is used in code
...
### ZACH COMMENTED THIS OUT FOR NOW!!
# import sympy
...
# import base64
from  forge import arg as forge_arg
from  forge import sign as forge_sign



def get_unit_quantity(name, base, scale_factor, abbrev=None, unit_system='SI'):
    '''Define a unit in terms of a base unit'''
    unit = sympy_units.Quantity(name, abbrev=abbrev)
...
def get_kamodo_unit_system():
    ...
    from sympy.physics.units import radian as sympy_radian
    from sympy.physics.units import degree as sympy_degree
...
def pad_nan(array):
    if len(array.shape) == 1:
        ...
        return pad(array.astype(float), (0, 1),
                      mode='constant', constant_values=nan).T
    elif len(array.shape) == 2:
        # print('padding array {}'.format(array.shape))
        return pad(array.astype(float), ((0, 1), (0, 0)),
...
def pointlike(_func=None, signature=None, otypes=[np_float], squeeze=None):
    ...
    def decorator_pointlike(func):
        def argument_wrapper(f, *args, **kwargs):
            ...
            for i, x in enumerate(args):
                if len(x.shape) == 1:
                    args[i] = expand_dims(x, axis=0)
            if squeeze is not None:
                try:
                    return vectorize(f, otypes=otypes, signature=signature)(*args, **kwargs).squeeze(squeeze)
                except:
                    return vectorize(f, otypes=otypes, signature=signature)(*args, **kwargs)
            else:
                return vectorize(f, otypes=otypes, signature=signature)(*args, **kwargs)
...
def solve(fprime=None, seeds=None, varname=None, interval=None,
    ...
    if len(seeds.shape) > 1:
        ...
        seeds = expand_dims(seeds, axis=0)

    t_eval = linspace(*interval, npoints)
    nseeds = len(seeds)
    ...
    def decorator_solve(f):
        ...
        for i, seed in enumerate(seeds):
            for d in directions:
                ...
                seed_numbers = ones(len(interval_bounded)) * i  # *len(directions) + 1*(d > 0)
        ...
        def solution(s=t):
            s = array(s)
            if len(s.shape) == 0:
                s = expand_dims(s, axis=0)

            isolution = floor(s.real).astype(int) * len(directions) + (s.imag > 0)

            results = []
            seed_number = []
            integral = []
            for soln, imag_ in zip(isolution, s.imag):
                seed_number.append(floor(soln / len(directions)))
                integral.append(imag_)
                try:
                    if isnan(np_abs(soln + imag_)):
                        results.append(ones(isolution.shape[-1]) * nan)
                    else:
                        results.append(solutions[soln](np_abs(imag_)))
                except:
                    results.append(ones(isolution.shape[-1]) * nan)
            index_ = MultiIndex.from_arrays([seed_number, integral],
                                               names=['seed', 'integral'])
            return DataFrame(vstack(results), index=index_).drop_duplicates()
...
def serialize(obj):
    from types import GeneratorType as types_GeneratorType

    if isinstance(obj, (ndarray, generic)):
        if isinstance(obj, ndarray):
            return {
        ...
        elif isinstance(obj, (bool_, number)):
            return {
    ...
    if isinstance(obj, Index):
        if isinstance(obj, DatetimeIndex):
            ...
            return {
                '__datetime__': [_ for _ in map(pd_datetime.isoformat, obj)],
                'dtype': full_classname(obj),
            }
        else:
            return {
    ...
    if isinstance(obj, types_GeneratorType):
...
def lambdagen(obj):
    ...
    for func_ in obj['__lambdagen__']:
        ...
        for arg, arg_default in func_['params'].items():
            signature.append(forge_arg(
                arg,
                default=deserialize(arg_default)))
        @forge_sign(*signature)
...
def deserialize(obj):
    ...
    if isinstance(obj, dict):
        if '__ndarray__' in obj:
            ...
            # ).reshape(obj['shape'])
            return array(obj['__ndarray__'])
        if '__npgeneric__' in obj:
            # return np.frombuffer(
            #     base64.b64decode(obj['__npgeneric__']),
            #     dtype=np.dtype(obj['dtype'])
            # )[0]
            return array(obj['__npgeneric__'])
        ...
        if '__datetime__' in obj:
            return to_datetime(obj['__datetime__'])
        if '__index__' in obj:
            return Index(obj['__index__'])
        if '__set__' in obj:
...
def get_undefined_funcs(expr):
    ...
    from sympy.function import AppliedUndef as sympy_AppliedUndef
    return expr.atoms(sympy_AppliedUndef)
...
def sign_defaults(symbol, expr, composition):
    ...
    for arg in symbol.args:
        ...
        if str_arg in defaults:
            ...
            arg_signatures.append(forge_arg(str_arg, default=arg_default))
        else:
            arg_signatures.append(forge_arg(str_arg))

    # will raise an error if defaults are not last
    signature = forge_sign(*arg_signatures)
    return signature
...
def get_bbox(fig):
    ...
    zmax = nanmax(t0['z'])
    for t in fig['data']:
        xmin = min(xmin, nanmin(t['x']))
        xmax = max(xmax, nanmax(t['x']))
        ymin = min(ymin, nanmin(t['y']))
        ymax = max(ymax, nanmax(t['y']))
        zmin = min(zmin, nanmin(t['z']))
        zmax = max(zmax, nanmax(t['z']))
