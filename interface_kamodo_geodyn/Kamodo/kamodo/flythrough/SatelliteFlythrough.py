...
#### ZACH ADD
import sys
sys.path.insert(0,'/data/geodyn_proj/interface_kamodo_geodyn/Kamodo/flythrough')
...
from kamodo.flythrough.SF_utilities import Prepare_Files,  MW 

#### ZACH only import parts of function we need.
# import numpy as np
...
def SatelliteTrajectory(dataset, start_ts, stop_ts, coord_type='GEO',
    ...
    '''
 #### ZACH-- Commment out unused codes for GEODYN CASE
...
def ModelFlythrough(model, file_dir, variable_list, sat_time, c1, c2, c3, 
    ...
    if isinstance(sat_time, list): sat_time = np_array(sat_time)
    if isinstance(c1, list): c1 = np_array(c1)
    if isinstance(c2, list): c2 = np_array(c2)
    if isinstance(c3, list): c3 = np_array(c3)
...
##### ZACH removed unused portions of code for the geodyn Case
#     print('Done.\n')
#     if verbose: 
