...
import time
# import numpy as np
#### Zach--- change to only import desired functions from numpy
...
#     print('Func: ConvertCoord_f()')
    
    #### Zach add Context:
        # According to Spacepy's Documentation, coordinate system; possible values are:
            # GDZ (Geodetic; WGS84),
...
#         print('   Func: Convertcoord.ts_to_spacepystr()')
...
#     @np.vectorize
#     def ts_to_spacepystr(ts):
#         print('   Func:-- L SHELL ts_to_spacepystr()')
#         return dt.strftime(dt.utcfromtimestamp(int(ts)), '%Y-%m-%dT%H:%M:%S')
