...
from kamodo.flythrough.SF_utilities import Prepare_Files, Single_FlyAway, MW, Single_FlyAway_GEODYN


def Prerun(model, file_dir, verbose=True):
    '''Determine vertical dependency and model time resolution of given model'''
    print('Main Func Main: Prerun()')
...
def SingleModelFlythrough(model, file_dir, variable_list, dz, sat_time, c1, c2, 
    ...
    ''' 
#     t0=perf_counter()
#     print('Main Func: SingleModelFlythrough()         ', perf_counter()-t0,'s')
...
#     print('sat_time',sat_time)
    ...
    results = Single_FlyAway_GEODYN(model, file_dir, variable_list, dz, sat_time, c1, 
...
#         if i<len(var_list):
#             file.write('\n' )
    ### ZACH made this simpler to send values to GEODYN
    file.write(f"{results['rho']:.20f} \n")
    file.write(f"{results['DRHODZ']:.20f}")

    

    file.close()
#     print('   *** KAMODO tiegcm RHO: ',results)
...
if __name__=='__main__':
    ...
    if len(argv)==2:  #first term is a string of a model name
        ...
        ##ZACH: Changed this from beig Unix time to YYMMDDHHMMSS #1426637500.0
        sat_time_yymmddhhmmss = argv[5]
        sat_time = datetime.timestamp(datetime.strptime(sat_time_yymmddhhmmss, '%y%m%d%H%M%S').replace(tzinfo=timezone.utc))
        sat_time = float(sat_time)
        
        c1 = float(argv[6]) #y[R_E] or lon[deg]
...
if __name__!='__main__': 
    print('import time:', perf_counter()-t0)