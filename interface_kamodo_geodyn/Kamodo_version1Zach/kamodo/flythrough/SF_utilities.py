...
#         print('ilev_grid',ilev_grid)
    ...
    #     print('rough_height',rough_height)
        ...
        height_res = 20.    #abs(height-max_height)
...
####### --------------------------------------------
#######  ZACH'S DIFFUSIVE EQUILIBRIUM EXTRAPOLATION
####### --------------------------------------------
...
def DIFF_EQUI_EXTRAP(results_dict, height_request ):  
    ...
    den_O1 = results_dict['psi_O'][i]  * results_dict['rho'][i]*1000.  # [kg/m^3]
    den_O2 = results_dict['psi_O2'][i] * results_dict['rho'][i]*1000.  # [kg/m^3]
    den_HE = results_dict['psi_He'][i] * results_dict['rho'][i]*1000.  # [kg/m^3]
    den_N2 = results_dict['psi_N2'][i] * results_dict['rho'][i]*1000.  # [kg/m^3] 
    ...
    results_dict['psi_O'][i]  = den_O1/(results_dict['rho'][i]*1000.)
    results_dict['psi_O2'][i] = den_O2/(results_dict['rho'][i]*1000.)
    results_dict['psi_He'][i] = den_HE/(results_dict['rho'][i]*1000.)
    results_dict['psi_N2'][i] = den_N2/(results_dict['rho'][i]*1000.)
...
def Model_SatelliteFlythrough(model, file_dir, variable_list, sat_time, c1, c2, 
    ...
    print('         Reading the TIEGCM data')#, perf_counter()-t0)
...
#         variable_list = ['rho','psi_O2', 'psi_O', 'psi_N2', 'psi_He', 'T_n']
    ...
    #perform flythroughs
    print('         Perform the Flythrough')#, perf_counter()-t0)
    for key in coord_dict.keys():
        ...
        ##### ZACH ADD check for presense of N2 in the file
        if "psi_N2" not in newvar_list:
            print('Will have to calc N2 manually')
            flag_add_psiN2 = True
            results_dict['psi_N2'] = np_zeros(np_size( results_dict['psi_O2']))
        else:
            flag_add_psiN2 = False
    ...
    print('         Correct with top boundary extrap. ')#, perf_counter()-t0)
...
#             rho_SIunit = results_dict['rho'][i]*1000.   #### CONVERT TO SI UNIT (kg/m^3)
            ...
            den_O2 = results_dict['psi_O2'][i] * results_dict['rho'][i]*1000.  # [kg/m^3]
            den_HE = results_dict['psi_He'][i] * results_dict['rho'][i]*1000.  # [kg/m^3]
            if flag_add_psiN2 == True:
                results_dict['psi_N2'][i] = 1 - (results_dict['psi_O'][i]  + 
                                                 results_dict['psi_O2'][i] + 
                                                 results_dict['psi_He'][i])
                den_N2 = results_dict['psi_N2'][i] * results_dict['rho'][i]*1000.
            else:
                den_N2 = results_dict['psi_N2'][i] * results_dict['rho'][i]*1000.  # [kg/m^3] 
            neutral_temp = results_dict['T_n'][i]      # neutral temperature [K]
...
#             pass
    ...
    print('         Done with top boundary extrap. ')#, perf_counter()-t0)
    return results_dict

    ##### ===================================================================
    ##### ZACH ADD Top Boundary Extrapolation        
   
#     alts_m     = results_dict['c3']*1000.
#     top_alts_m = results_dict['max_height']
#     dens_O1 = results_dict['psi_O'] *results_dict['rho']*1000.  # [kg/m^3]
#     dens_O2 = results_dict['psi_O2']*results_dict['rho']*1000.  # [kg/m^3]
#     dens_HE = results_dict['psi_He']*results_dict['rho']*1000.  # [kg/m^3]
#     dens_N2 = results_dict['psi_N2']*results_dict['rho']*1000.  # [kg/m^3] 
#     temps    = results_dict['T_n']
#     rhos_SIunit  = results_dict['rho']*1000.# [kg/m^3]
    
#     results_dict =  [DIFF_EQUI_EXTRAP(results_dict, alt, ) \
#                       for alt, top_alt, rhoO1, rhoO2, rhoHE, rhoN2, temp, rho   in zip(
#           alts_m, top_alts_m, dens_O1, dens_O2, dens_HE, dens_N2, temp, rho_SIunit) \
#                          if alts_m > top_alt]
#     results_dict =  [DIFF_EQUI_EXTRAP(results_dict, alt ) \
#                       for alt, top_alt   in zip(alts_m, top_alts_m) \
#                          if alt > top_alt]
