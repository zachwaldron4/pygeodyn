...
def day_files(file_pattern, reader, call_type):
    '''Retrieve file times. Convert files if necessary.'''
        
    print('Func: day_files()')
...
def check_timescsv(file_pattern, reader, model, call_type='normal'):
    '''check for times csv file, write if not found in file_dir'''
            
    print('Func: check_timescsv()')
    ...
    if not isfile(csv_filename):
        times = day_files(file_pattern, reader, call_type)
        write_timescsv(csv_filename, times)
    else:
        times = read_timescsv(csv_filename)
    return times

def save_times(file_patterns, sat_time, reader, model, verbose=False):
    ...
    print('Func: save_times()')
    

    times = check_timescsv(file_patterns, reader, model) 
    #print(times)
...
''' 
 #### Zach edit: changed the below to be verbose=False
def CalcIlev(H, Hunit, t, c1_val, c2_val, height, ilev_grid, z_unit, high_res, verbose=False):
...
def coordinate_systems(model, sat_time, c1, c2, c3, variable_list, coord_type, coord_grid):
    ...
    if len(var_coord_strs)!=1 or var_coord_strs[0]!=(coord_type+','+coord_grid):  #then coordinate conversion needed
        ...
        for key in new_coords.keys():   #e.g. key= 'GDZ,sph'
            #convert to needed coordinates, in/out order depends on coord_grid
...
def Model_SatelliteFlythrough(model, file_dir, variable_list, sat_time, c1, c2, 
    ...
    print('Func: Model_SatelliteFlythrough()')
    ...
    sat_time, times, net_idx = save_times(file_patterns, sat_time, reader, model, verbose=verbose)
...
def Prepare_Files(model, file_dir, call_type='normal'):
    ...
    times = check_timescsv(file_patterns, reader, model, call_type=call_type)  #reader creates converted files if DNE
    print('Files prepared for run.')
    return 

#----------Code below is for Single Satellite Flythrough software--------------------------------

def find_singlefiletime(file_patterns, sat_time, reader, model):
    '''Find file containing given single time. Adjust if within dt seconds.'''
            
    print('Func: find_singlefiletime()')
    

    t_time = perf_counter()
    times = check_timescsv(file_patterns, reader, model, call_type='single')  #Retrieve file times
...
def Single_FlyAway(model, file_dir, variable_list, dz, sat_time, c1, c2, c3, 
    ...
    #time_array.append(['test0b',perf_counter()-t0])
    filename = find_singlefiletime(file_patterns, sat_time, reader, model)
    #time_array.append(['test0',perf_counter()-t0])
    #t1 = perf_counter()
