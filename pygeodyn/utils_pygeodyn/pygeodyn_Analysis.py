...
def Calc_Cd_percent_diff_apriori(obj_m1):
    ...
    SAT_ID = int(obj_m1.__dict__['run_parameters']['SATID'])
...
#     print('Avg % diff for ', model_m1 ,': ',avg_percdiff_cd_m1,'%')
...
#     obj_m1_stats['avg_percdiff_cd_m1'] = avg_percdiff_cd_m1
    return(obj_m1_stats)
...
#     fig = go.Figure()
    ...
    for i,arc in enumerate(arc_list):
        ...
        fig.add_trace(go.Scattergl(x=obj_m1.Residuals_obs[arc]['Date'][::3],
                                 y=obj_m1.Residuals_obs[arc]['Residual'][::3],
                                 name= 'Arc: '+str(i_arc),
                                 mode='markers',
                                 marker=dict(color=col,
                                 size=3,),
