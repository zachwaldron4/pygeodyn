...
#### Notes from Zach:
        #   I'm not totally sure as of 8/12 but i dont think these should all be in converted to geodetic coordinates.
        #   I added a fix for the specific cases I need, but a more formal fix of the coordinate systems will be required.  
            # Leaving these in SPH coordinates is correct (to my undertanding) 
            #  and they should not NEED a coordinate transformation, however 
            #  the spherical coordinates as they are set up now returns the 
            #  ALT unit as RE which I dont believe is correct, it should return as KM.


#     ZACH TO DO:  NEED TO FIX THE ILEV vs ILEV1 issues
...
model_varnames={
                 ...
                 "TE"  : ["T_e",'variable description',9,'GDZ','sph',['time','lon','lat','ilev'],"K"], #  ELECTRON TEMP ,
                 "TI"  : ["T_i",'variable description',10,'GDZ','sph',['time','lon','lat','ilev'],"K"], #  ION TEMP E
                ...
                #### ZACH EDIT!,    I think this should be made SPH instead of GDZ
...
def min_to_date(time_minutes, correct_REF_Date):
    '''Use minutes since 2000-03-20 00:00:00 to determine the date in the file. Returns a string and a datetime object.'''
    
    date_string = datetime.strftime(min_to_dt(time_minutes, correct_REF_Date), '%Y-%m-%d')  #'YYYY-MM-DD'
    return datetime.strptime(date_string, '%Y-%m-%d').replace(tzinfo=timezone.utc)


def min_to_dt(time_minutes, correct_REF_Date):  #called by functions below
    '''Convert minutes since 2000-03-20 00:00:00 to datetime object in UTC'''
    
    return correct_REF_Date.replace(tzinfo=timezone.utc)+timedelta(minutes=time_minutes)
        
def min_to_dts(time_minutes,correct_REF_Date):
    '''Convert minutes since 2000-03-20 00:00:00 to a datetime string'''
    
    return datetime.strftime(min_to_dt(time_minutes, correct_REF_Date), '%Y-%m-%d %H:%M:%S')


@vectorize
def min_to_hrs(time_minutes, date_dt, correct_REF_Date):
    '''Convert  minutes since 2000-03-20 00:00:00 to hours since midnight using predetermined datetime object.'''
    
    return (min_to_dt(time_minutes, correct_REF_Date)-date_dt).total_seconds()/3600.
...
#     print('KAMODO IMPORTED!')
    ...
    class MODEL(Kamodo): 
        ...
        def __init__(self, full_filename, variables_requested=[], runname="noname",
            ...
            ##### ===========================================================================================
            #### ZACH FIX:
            #####    The files are not supposed to be some time since 2000-03-20 00:00:00 UTC,
            ####      I think there is an error in what the units are saying the date is...
            #####     We should index the content to determine what the date is
#             YYDDD_0 = cdf_data.getncattr('contents').split()[0]       # start time
#             YYDDD_1 = cdf_data.getncattr('contents').split()[5]       # end time
#             DT0 = datetime.strptime(YYDDD_0, '%y%j').replace(tzinfo=timezone.utc)
#             DT1 = datetime.strptime(YYDDD_1, '%y%j').replace(tzinfo=timezone.utc)
#             DT_str_0 = datetime.strftime(DT0, '%Y-%m-%d %H:%M:%S')
#             DT_str_1 = datetime.strftime(DT1, '%Y-%m-%d %H:%M:%S')
#             delta_mins = cdf_data.getncattr('contents').split()[10]   # step size in minutes
#             self.dt = int(delta_mins)*60
#             self.datetimes =  [DT_str_0, DT_str_1]   #pd_date_range(start=DT0 ,end=DT1, freq = delta_mins+'min')
#             self.filedate = self.datetimes[0] # First start time
#             self.filetimes=[dts_to_ts(file_dts) for file_dts in self.datetimes]   #timestamps in UTC 
            
            #### The below is the correct reference date info
            correct_ref_DoY = cdf_data['time'].initial_day  # The correct referece Day of Year
            correct_ref_Year = datetime.strptime(cdf_data.getncattr('contents').split()[0], '%y%j').replace(tzinfo=timezone.utc).year
            correct_REF_Date = datetime.strptime(str(correct_ref_Year)+'-'+str(correct_ref_DoY), '%Y-%j')
            
            ##### ===========================================================================================
            ### ----------------------------------------------------------------------------------------------
            ### Zach add: added the reference date to the time functions so that we get the correct date.
            ### RR's Original Time Stuff:
            time = array(cdf_data.variables['time'])  #in minutes since 2000-03-20 00:00:00 UTC
            self.filedate = min_to_date(time[0], correct_REF_Date)  #datetime object for file date at midnight UTC
            self.datetimes = [min_to_dts(time_minutes, correct_REF_Date) for time_minutes \
                              in [time[0], time[-1]]]  #strings in format = YYYY-MM-DD HH:MM:SS
            self.filetimes=[dts_to_ts(file_dts) for file_dts in self.datetimes]   #timestamps in UTC 
            self.dt = diff(time).max()*60.  #time is in minutes
            ### ----------------------------------------------------------------------------------------------
#             print('Check-- TIEGCM_4D, self.filedate', self.filedate)
#             print('Check-- TIEGCM_4D, self.datetimes', self.datetimes)
#             print('Check-- TIEGCM_4D, self.filetimes', self.filetimes)
            ...
            if filecheck:  #new_time iis a utc timestamp
                ...
                self._time = insert(min_to_hrs(time, self.filedate, correct_REF_Date), 0, new_time) #convert to hours since midnight

            else: 
                self._time = min_to_hrs(time, self.filedate, correct_REF_Date)
...
#             print('T4Dr Func: register_3D_variable() ')
...
#             print('T4Dr Func: register_4D_variable()  ')
