...
"""
### Zach changed the imports to only import specific functions that are used
#     - Remove plotting stuff to reduce overhead in the packages
#     - Reduce overhead by only importing specific functions from packages
...
def parse_expr(*args, **kwargs):
    try:
        return sympy_sympify(*args, **kwargs)
    except:
...
def get_str_units(bracketed_str):
    ...
    """
    return re_findall(r"\[([A-Za-z0-9_/*^\\(\\)-\.]+)\]", bracketed_str)
...
def extract_units(func_str):
    ...
    if str_has_arguments(func_str):
        ...
        lhs_args = re_findall(r"\((.+)\)", func_str)[0]
...
class Kamodo(UserDict):
    ...
    def evaluate(self, variable, *args, **kwargs):
        ...
        if isinstance(self[variable], vectorize):
    ...
    def solve(self, fprime, interval, y0,
        ...
        varname = next(iter(inspect_signature(self[fprime]).parameters.keys()))
...
class KamodoAPI(Kamodo):
    ...
    def __init__(self, url_path, **kwargs):
        ...
        self.__doc__ = requests_get('{}/doc'.format(self._url_path)).text
        ...
        for k, v in self._kdata.items():
            ...
            func_doc = requests_get(doc_path).text
    ...
    def _get(self, url_path):
        req_result = requests_get(url_path)
        try:
    ...
    def load_func(self, func_name):
        ...
        for arg, arg_default in self._defaults[func_name].items():
            signature.append(forge_arg(arg, default=arg_default))
