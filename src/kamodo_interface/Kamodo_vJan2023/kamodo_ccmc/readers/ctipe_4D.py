...
## Zach Change the Coord system to  for necessary vars
model_varnames = {'rho': ['rho', 'total mass density',
                          0, 'GDZ', 'sph', ['time', 'lon', 'lat', 'ilev1'], 'kg/m**3'],
                  'T': ['T', 'temperature',
                        1, 'GDZ', 'sph', ['time', 'lon', 'lat', 'ilev1'], 'K'],
...
def MODEL():
    ...
    class MODEL(Kamodo):
        ...
        def __init__(self, full_file_prefix, variables_requested=[],
            ...
            # lon = array(cdf_data.variables['lon_n'])
            ...
            # self._lon0 = lon_ge180+lon_le180
            # print(self._lon0)
            # print(lon)

            # lon = array(cdf_data.variables['lon_d'])
            # lon_le180 = list(where(lon <= 180)[0])  # 0 to 180
            # lon_ge180 = list(where(lon >= 180)[0])
            # # tmp = append(lon, 360.) - 180.  # now -180. to +180. -- this works as expected for a grid with 0 and 180 as grid positions
            # self._lon1 = lon_ge180+lon_le180

            self._ilev = array(cdf_data.variables['ilev'])  # neutral file
            self._lon0 = array(cdf_data.variables['lon_n'])#-180
            # self._lon0 =  np_sort([val-360 if val>180  else val \
                                #    for val in array(cdf_data.variables['lon_n'])])
            ...
            self._lon1 = array(cdf_data.variables['lon_d'])#-180.
            # self._lon1 =  np_sort([val-360 if val>180  else val \
            #                        for val in array(cdf_data.variables['lon_d'])])
            # self._lon1_idx =  list(np_where(array(cdf_data.variables['lon_d'])>=180)[0]) \
            #                 + list(where(array(cdf_data.variables['lon_d'])<=180)[0])

              #np_argsort([val-360 if val>180  else val \
                        #for val in array(cdf_data.variables['lon_d'])])
        ...
        def register_4D_variable(self, units, variable, varname, gridded_int):
            ...
            if 'radius' in coord_list:
                ...
                xvec_dependencies = {'time': 'hr', 'lon': 'deg', 'lat': 'deg',
                                     'radius': 'R_E'}
            # print('ZACH-- register_4D_variable')
            ...
            if varname[0:3] == "rho" or varname[0:2] == "N_":
                log_data = np_log(variable)
                self = regdef_4D_interpolators(self, units, log_data, self._time,
                                           lon, lat, z, varname,
