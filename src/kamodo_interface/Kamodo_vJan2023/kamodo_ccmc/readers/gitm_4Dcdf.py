...
#             print('zach-- total_files',total_files)
            
            if len(file_prefix.split('_')[-1]) == 2:  # asking for hourly files
#                 if verbose:
#                     print('# asking for hourly files')
                self.patterns = unique([basename(f)[:16] for f in total_files
                                        if basename(f)[13:16] != '.nc' and
                                          ...
                                          'data'  not in basename(f)   ])   # Zach modified for gitm2 case
                day_tag = False
            else:  # asking for daily files (satellite flythrough)
#                 if verbose:
#                     print('# asking for daily files (satellite flythrough)')
...
#             print('zach-- self.patterns',self.patterns)
            for name in self.patterns:
#                 print('zach -- gitm_4Dcdf name: ', name)
                                
                if '3DLST' in name or '3DALL' in name:
                    cdf_file = name  # need to get reference height correct
#                     print('zach -- gitm_4Dcdf', cdf_file)
