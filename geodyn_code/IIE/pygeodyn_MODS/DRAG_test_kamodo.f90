...
! READ IN THE MSIS OPTIONS but we only want to have to do this once...
      if(kentry.eq.1)then
          ...
          do i=1,2
!             if(kentry.eq.1)then
                WRITE(6,*) 'CHECK COUNT OPTIONS i/o times'
!             endif
...
!
      ...
      IF(IATDN.EQ.5) THEN
!
!        ....CALL TO MSIS86 option INSTEAD OF DTM
!
!
!
! MSIS case
          ...
          select case (choose_msis)
            case(0)
             ...
             &       I324,IDRV,RHO,DRHODZ,IERR)
                 C(1)=DRHODZ
                  if(kentry.eq.1)then
                      WRITE(6,*) 'CHECK- DRAG.f90: using msis86'
                  endif
! MSIS 00 case
            case(1)
                 IMARK = IND3HR
             ...
             &         I324,IDRV,RHO,DRHODZ,IERR,optionsin)
                 C(1)=DRHODZ    
                 if(kentry.eq.1)then
                      WRITE(6,*) 'CHECK- DRAG.f90: using msis00'
                 endif
! MSIS 2 case
          case(2)
                 IMARK = IND3HR
             ...
             &         I324,IDRV,RHO,DRHODZ,IERR,optionsin)
                 C(1)=DRHODZ          
                 if(kentry.eq.1)then
                      WRITE(6,*) 'CHECK- DRAG.f90: using msis2'
                 endif
            ...
            case(3)   ! add a CASE 3 that represents using Kamodo! NOT MSIS
                 ...
                 if(kentry.eq.1)then
                      WRITE(6,*) 'CHECK- DRAG.f90: KAMODO-ctipe'
                 endif
                 
                 CALL Kamodo_CTIPe(MJDSEC,ALTI,XLATD,XLOND,          &
             &         RHO,DRHODZ)
                 C(1)=DRHODZ          
                 if(kentry.eq.1)then
                      WRITE(6,*) 'CHECK- DRAG.f90: KAMODO-ctipe'
