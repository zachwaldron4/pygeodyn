...
C 
C     INPUT VARIABLES:
C        IYD - YEAR AND DAY AS YYDDD (day of year from 1 to 365 (or
C        366))
...
C        and GTD7D
C
C          SUBROUTINE GTD7 -- D(6) is the sum of the mass densities of
C          the
C          species labeled by indices 1-5 and 7-8 in output variable D.
C          This includes He, O, N2, O2, Ar, H, and N but does NOT
C          include
...
C
C      IMPLICIT DOUBLE PRECISION (A-H,O-Z),LOGICAL(L)
C      SAVE
C
...
C      Put identification data into common/datime/
    ...
    1 CONTINUE
C ZACH ADD PRINT
C*  PRINTOUT FOR TESTING.
C
C  IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T
C
C     PRINT 813,IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T
C 813 FORMAT(1X, '    ',/,                                             
C      &      1X, '     INSIDE subroutine GTD7:  ',I8,1X, F10.4,/,     
C      &      1X, '     ALTkm: ',F12.3,/,                               
C      &      1X, '     LAT,LON: ',2(F9.3,1X),/,                        
C      &      1X, '     LOC TIME: ',F8.5,/,                             
C      &      1X, '     FLUXAVG = ',F6.2,' VS. ','DAILY FLUX: ',F6.1,/, 
C      &      1X, '     AP = ',7F6.2,/,                                 
C      &      1X, '     MASS = ',I8 ,/,                                 
C      &      1X, '     DEN: = ',9(E15.8,1X),/,                         
C      &      1X, '     TEMP = ',2(E15.8,1X),/,                         
C      &      1X, '     ' )
C
      DO 2 I=1,2
        ISTIME(I)=IST(I)
...
C
      WRITE(6,813) IYD,SEC
  813 FORMAT(1X,'     INSIDE subroutine GTD7:  ',I8,1X, F10.4)         
      WRITE(6,814) ALT,GLAT,GLONG
  814 FORMAT(1X,'     ALT: ',F12.3,'  GLAT: ',F9.3,'  GLONG: ',F9.3)
      WRITE(6,815) STL,F107A,F107
  815 FORMAT(1X,'     STL: ',F8.5, '  F107A: ',F6.2,'  F107: ',F6.2)
      WRITE(6,816) AP
  816 FORMAT(1X,'     AP: ',7F6.2)
      WRITE(6,817) MASS
  817 FORMAT(1X,'     MASS: ',I8)
      WRITE(6,818) D
  818 FORMAT(1X,'     DEN: ',9(E15.8,1X))
      WRITE(6,819) T
  819 FORMAT(1X,'     TEMP: ',2(E15.8,1X))
C
C
C
C
...
C            1X, '     TEMP = ',2(E15.8,1X),/,                           &
C            1X, '     ' )
C1X,29HGHP7 NOT CONVERGING FOR PRESS, 1PE12.2,E12.2)
C
C
C      PRINT 813,IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T
C      813 FORMAT(1X, '    ',/,                                          &
...
C         Only calculate nodes if input changed
     ...
     $  *TN2(4)*TN2(4)/(PMA(1,3)*PAVGM(3))**2
        TN3(1)=TN2(4)
       ENDIF
C Including ZN3(1) in the jump condition creates a model coverage gap at
C that exact altitude
...
C
   ...
   10 CONTINUE
      GOTO 90
   50 CONTINUE
C      DD=DENSM(ALT,1.,0,TZ,MN3,ZN3,TN3,TGN3,MN2,ZN2,TN2,TGN2)               
C ZACH 
...
C       Correction to specified mixing ratio at ground
   ...
   38 CONTINUE
C      Correction for general departure from diffusive equilibrium above
C      Zlb
...
C       Calculate Temperature and Density Profiles for lower atmos.
      ...
      DATA RGAS/831.4/
C
C     ZACH   DD=DENSM(ALT,1.,0,TZ,MN3,ZN3,TN3,TGN3,MN2,ZN2,TN2,TGN2)
C     Warning: Type mismatch in argument 'xm' at (1); passed INTEGER(4) to REAL(4)
C      INTEGER XM
