!$BARYCX
      SUBROUTINE BARYCX(MJDSEC,FSEC,XPC,VPC,NDXVPC,                     &
...
!                         INTRODUCED FOR THE SUPPLEMENTARY EPHEMERIS
!
! I/O PARAMETERS:
!
...
!********1*********2*********3*********4*********5*********6*********7**
     ...
     &          XPC(NDXVPC,3),VPC(NDXVPC,3),PARCBD(NM,36),PARTBD(NM,36)
      DIMENSION TTT(NM),TTTMO(NM),FNSEC(4),TWOT(NM),VSEC(988)
      DIMENSION AA(1),II(1)
...
! INITIALIZE
  ...
  100 END DO
!
! THE CODE BELOW NEEDS TO BE FURTHER MODIFIED IF THE EARTH AS A TRACKING
! BODY IS FEATURED IN THE SUPPLEMENTARY EPHEMERIS.
...
! CENTRAL BODY
!
! CHECK HERE IF THE BLOCK SPANS TWO SUPP EPHEM BUFFERS AND RESET HERE
! THE TIMES
...
!     write(6,*)'dbg PSTAT',PSTAT(K,I),k,i,COEFX(I,J),i,j,CHEB(K,J),k,j
 1600 CONTINUE
!
! COMPUTE VELOCITY
      IF(LVEL.OR.LCNVEL) THEN
...
!  --------------------------------------------------------------
!
!     write(6,*)' dbg ECORR ',ECORR(1),EMJDS0(1)
