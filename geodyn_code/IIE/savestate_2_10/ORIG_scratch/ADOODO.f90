!$ADOODO
      FUNCTION ADOODO(COSEPH,SINEPH,NN,MM,KK,HH,JJ,IBDY,SIGN1,FI,GE)
!***********************************************************************
...
!            OTIDE TIDAL EXPANSION MODEL FOR SOLID OCEAN TIDES (SEE
...
! CHECK SYMMETRY
!
! TWO-FOLD SYMMETRY WHEN:
!     1) K=0
!     2) M=0
!     3) H=1,K=0,J=0
!
! FOUR-FOLD SYMMETRY WHEN
!     1) M=0,K=0
!
! SYMMETRY DEPENDENT ON WHICH MODEL IS USED
!   OTIDEN CARDS TAKE M & K SYMMETRY INTO ACCOUNT INTERNALLY
!   OTIDES CARDS TAKE M & K SYMMETRY INTO ACCOUNT EXTERNALLY
!   BOTH TAKE H=1, J=0, K=0 SYMMETRY INTO ACCOUNT INTERNALLY
!   DODI VALUES INIT. IN AANDBO TAKE SYMMETRY INTO ACCT.
!   EXCEPT K1,K2 FOR H=1,J=0,K-0
!
      INDEX=ABS(JJ)+KK
      IF(NUNPLY .EQ. 1) THEN
      IF(KK.EQ.0)W=TWO*W
...
!
 ...
 2100 CONTINUE
      ...
      SUM=B(1)*FI(1)*GE(1)+B(2)*FI(2)*GE(2)
 2200 CONTINUE
      ADOODO=W*(2.0D0-DM)*S*FACT(MM1)*SUM/FACT(MM2)
 3000 CONTINUE
      RETURN
      END
