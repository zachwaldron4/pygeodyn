      SUBROUTINE ALTPT(AA,II,LL,NM,MJDSBL,FSEC,ISAT,ISET1,OFF,TRK,SCR,  &
     &                 XSM,VSM,ITERP,ATROT,HOLD,IDA,FRQION)
!***********************************************************************
!
...
!
      ...
      WRITE(6,*)'EXECUTION TERMINATED IN ALTPT   '
      STOP 16
      ENDIF
...
!        print *,'altpt: i,jxyzof: ',i,aa(JXYZOF(1)-1+I)
!         print *,'altpt: i,jxyzcg: ',i,aa(JXYZCG(1,1)-1+I)*oscale
!         print *,'altpt: i,jxyzcg 2: ',i,aa(JXYZCG(2,1)-1+I)*oscale
  100    CONTINUE
      ENDIF
!
...
!!!!        ELSE
     ...
     &              PITCH,YAW,TDIF,SINWT,COSWT,.TRUE., .FALSE., XSM, VSM)
!!!!        ENDIF
!      write(6,*)' dbg after getrpy ',ATROT(1,1),ATROT(1,2),ATROT(1,3)
!      write(6,*)' dbg after getrpy ',ATROT(2,1),ATROT(2,2),ATROT(2,3)
!      write(6,*)' dbg after getrpy ',ATROT(3,1),ATROT(3,2),ATROT(3,3)
       ENDIF
!
...
!
      IF(LOFEXT(1)) THEN
        ...
        IF(.NOT.LL(KEALQT+ISET1-1).AND..NOT.LSIMND) THEN
           WRITE(6,6000)
           WRITE(6,6001)
           WRITE(6,6002)
           WRITE(6,6003)
...
!       ENDIF FOR LXATTD
     ...
     &              AA(KPHC),XYOF31,ISATID,AA(KSTAIN),II,dummy,lpre,    &
     &              LDORANT,.FALSE.)
        LXATTD=LXAH
...
! ANALYTICAL ATTITUDE MODELS
      IF(ICRSYS.LT.0.OR.ICRSYS.GT.20) THEN
          WRITE(6,6000)
          WRITE(6,6001)
          WRITE(6,6005) ICRSYS
          WRITE(6,6006)
     ...
     &               .FALSE.,.FALSE.,DUMA,LL(KLANTC),II(KANCUT),        &
     &               .FALSE.,LWND,DUMA,1,1,AA(KSTAIN),AA(KANTBL),       &
     &                LDORANT)
!!!    ....added CORL02 common block because KLTPMS was undefined ! jjm
!!!    ....added CORA02 common block because KTPMES was undefined ! jjm
      ENDIF
      IF(ICRSYS.EQ.3) THEN
...
!altpt  CALL TRKTOP(XSM,VSM,OFF,OFFSET,SCR,FSEC,                       &
!     &               TRK,SCR,LL(KLPTS),AA,II,II,                        &
!     &               AA(KTPMES+4*MINTIM),LL(KLTPMS+6*MINTIM),           &
!     &               .FALSE.,1,ISATID,.TRUE.,MINTIM,ATROT,DUMA,.FALSE., &
...
!                       IDNW,ANTTAB)
      ...
      IF(ICRSYS.EQ.15) THEN
          CALL TRKCR2(XSM,VSM,OFF,OFFSET,FSEC,TRK,SCR,AA,II,ISATID,     &
     &                .TRUE.,LOFFA1,AA(KOFDRV),.TRUE.,MINTIM,ATROT,     &
     &                LL(KLANTC),II(KANCUT),AA(KSTAIN),ISTAID,DUMA,     &
     &                AA(KANTBL),LDORANT)
      ENDIF

      IF (ICRSYS.EQ.18) THEN
      CALL TRKHY2(XSM,VSM,OFF,OFFSET,FSECSA,TRK,SCR,AA,II,.TRUE.,       &
     &             NM,ATROT,LL(KLANTC),II(KANCUT),ISATID,AA(KSTAIN),    &
     &             ISTAID,DUMA,AA(KANTBL),LDORANT,AA(KOFDRV),LOFFA1)
      ENDIF

      IF (ICRSYS.EQ.19) THEN
      CALL TRKSRL(XSM,VSM,OFF,OFFSET,FSECSA,TRK,SCR,AA,II,.TRUE.,       &
     &         MINTIM,ATROT,LL(KLANTC),II(KANCUT),ISATID,AA(KSTAIN),    &
     &             ISTAID,DUMA,AA(KANTBL),LDORANT,AA(KOFDRV),LOFFA1,    &
     &             .FALSE.)
      ENDIF
...
!
! Assume unit vector on data is actually true pointing and not apparent pointing
! True pointing is corrected for the spacecraft velocity abberation
! Our 2-way geolocation algorithm needs apparent pointing.
! Therefore convert true pointing unit vector passed through data to apparent po
!
! debug
!      print *,'altpt: lxattd, ldvct: ',lxattd, ldvct
! debug
      IF(LXATTD .AND. LDVCT) THEN
       DO I = 1, NM
        OFF(I,1) = OFF(I,1) + VSM(I,1)/VLIGHT
        OFF(I,2) = OFF(I,2) + VSM(I,2)/VLIGHT
        OFF(I,3) = OFF(I,3) + VSM(I,3)/VLIGHT
        OMAG = SQRT(OFF(I,1)**2+OFF(I,2)**2+OFF(I,3)**2)
        OFF(I,1) = OFF(I,1)/OMAG
        OFF(I,2) = OFF(I,2)/OMAG
        OFF(I,3) = OFF(I,3)/OMAG
       ENDDO
! debug
!       vmag = sqrt(vsm(1,1)**2+vsm(1,2)**2+vsm(1,3)**2)
!       omag = sqrt(off(1,1)**2+off(1,2)**2+off(1,3)**2)
!       print *,'altpt: i=1 vmag, omag: ',vmag,omag
!       print *,'altpt: vlight: ',vlight
! debug
      ENDIF
      RETURN
 6000 FORMAT(' ')
 6001 FORMAT(' EXECUTION TERMINATING IN SUBROUTINE ALTPT.')
