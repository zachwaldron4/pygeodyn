!$PSUBNP
      SUBROUTINE PSUBNP(ICON  ,DXSDPM,COSTHG,SINTHG,PMPXI ,XM    ,      &
     &    PXSPXP,PXSPLV,NMP   ,NADJP ,NM    ,NINTPL,                    &
     &    PMPXE ,PMPA  ,ISTA  ,LESTA ,LESTP ,ISTAMT,PXSPAB)
!********1*********2*********3*********4*********5*********6*********7**
! PSUBNP           00/00/00            0000.0    PGMR - ?
!
!
! FUNCTION:
!
...
! CALCULATE STARTING LOCATION IN NORMAL MATRIX
      ...
      DO 1000 I=1,3
      DO 800 N=1,NM
      PMPA  (INDEX,N)=PMPA  (INDEX,N)+PMPXE (N,I)*DXSDPM(J,I,ISTA)
  800 END DO
 1000 END DO
      INDEX=INDEX+1
 2000 END DO
...
! POLAR MOTION
      ...
      DO 3000 I=1,3
      DO 2800 N=NM1,NM2
      PMPA  (INDEX,N)=PMPA  (INDEX,N)+PMPXE (N,I)*PXSPXP(N,I,J)
 2800 END DO
 3000 END DO
      INDEX=INDEX+1
 4000 END DO
! U.T.
      DO 5000 I=1,2
      DO 4800 N=NM1,NM2
      PMPA  (INDEX,N)=PMPA  (INDEX,N)+PMPXE (N,I)*PXTPUT(I)
 4800 END DO
 5000 END DO
 6000 CONTINUE
      NM1=NM2+1
...
! STATION TIDAL DISPLACEMENT COEFFICIENTS H2 & L2
      ...
      DO 8000 I=1,3
      DO 7800 N=1,NM
      PMPA  (INDEX,N)=PMPA  (INDEX,N)+PMPXE (N,I)*PXSPLV(N,I,J)
 7800 END DO
 8000 END DO
 9000 IXLOVE=IXL2LV
! OCEAN  LOADING
...
! I--> XYZ OF STATION
      DO 11000 I=1,3
      DO 10000 N=1,NM
      PMPA  (INDEX,N)=PMPA  (INDEX,N)+PMPXE(N,I)*PXSPAB(N,I,JNDEX)
