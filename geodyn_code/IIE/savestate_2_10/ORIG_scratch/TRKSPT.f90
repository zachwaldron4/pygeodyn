!$TRKSPT
      SUBROUTINE TRKSPT(XSM,VSM,UI,OFFSET,FSECRA,RA,DR,AA,II,ISATID,    &
     &                  LALT,NDIMRA,ATROT,LANTCT,IANTSC,STAINF,INDSTA,  &
     &                  PHC,ANTTAB,DWRKDO,LOFFAJ)
!********1*********2*********3*********4*********5*********6*********7**
...
!--------------------------------------------------------------------
      ...
      !      'trkspt:1 MTYPE, XGPSTB      ', MTYPE, XGPSTB
      !write(6,'(A,1x,I4,1x,F6.1)') &
      !      'trkspt:1 ISEQ, XANTSL(ISEQ) ', &
      !                ISEQ, XANTSL(ISEQ)
      ...
      if( XGPSTB .GT. 0.D0  .or.                     &
          ...
          !write(6,'(A,1x,I4,1x,F6.1)') &
          !      'trkspt:2 MTYPE, XGPSTB            ', MTYPE, XGPSTB

          IANT=(XANTSL(ISEQ)+.001D0)

          !write(6,'(A,1x,I4,1x,F6.1, 1x, I6)') &
          !      'trkspt:2 ISEQ, XANTSL(ISEQ), IANT ', &
          !                ISEQ, XANTSL(ISEQ), IANT

      endif ! xgpstb...

!      write(6,'(A,1x,I4,1x,I6, 4x, L1)') 'trkspt: ISEQ, IANT, LPHC ', &
!                                                  ISEQ, IANT, LPHC
!      write(6,'(A,1x,I10)')  'trkspt: ISATID ', ISATID
...
!                 'trkspt: ISATID, iant, index_sat, L_sat_phc', &
...
!                 'trkspt: IANT, index_ant, L_ant_phc', &
!                          IANT, index_ant, L_ant_phc
      ...
      !write(6,'(A,3x,L1)') 'trkspt: LPHC ', LPHC
      ...
      IF( LPHC ) THEN
        ...
        if( NANTPT > 0 .and. L_ant_phc )then
            ...
            !write(6,'(A,4(1x,I6))') &
            !      'trkspt:a IANT, IPOFF, NDIMA, NDIMZ ', &
        ...
        if( L_sat_phc )then
            ...
            !write(6,'(A,4(1x,I6))') &
            !      'trkspt: sa index_sat, IPOFF, NDIMA, NDIMZ ', &
...
!      write(6,'(A,3(1x,I4))') 'trkspt: MTYPE, ISEQ ', &
!                                       MTYPE, ISEQ
!      write(6,'(A,3x,L1)') 'trkspt: LPHC ', LPHC
...
! COMPUTE INERTIAL OFFSET UNIT VECTOR
      DO 100 I=1,NM
        ...
        VPOS(2) = VSM(I,2)
        VPOS(3) = VSM(I,3)
        CALL SPOTAT(MJDSBL,FSECRA(I),XPOS,VPOS,BDFOFF(1),               &
     &              BDFOFF(2),BDFOFF(3),UNTOFF(1),UNTOFF(2),            &
     &              UNTOFF(3),DUM,1,0,.FALSE.,ISATID,II(KIDATB),        &
     &              AA(KSABIA),AA(KSBTM1),                              &
     &              AA(KSBTM2),AA(KVLOUV),II(KNSTLV),II(KSLVID),        &
     &              AA(KTSLOV),AA,IDUM,ROT,ATROT(1,1,I))
      IF(LOFFAJ) THEN
...
!-------------------------------------------------------------------------------
      ...
      IF( LPHC ) THEN
          ...
          if( i == 1 )then
              ...
              !      'trkspt: XV, YV, ZV ', XV, YV, ZV
              !write(6,'(A,2(1x,I10))')  'trkspt: I, IMQP ', i, imqp

              !if( NANTPT > 0 .and. L_ant_phc )then
              !    write(6,'(A,3(1x,I10))') &
              !          'trkspt: iant, kantps(iant), ipoff ', &
              ...
              if( L_sat_phc )then
                  ...
                  !      'trkspt: sa ANTTAB(IPOFF+3) , NDIMA   ', &
                  !                  ANTTAB(IPOFF+3) , NDIMA
                  !write(6,'(A,1x,E15.7,1x,I10)') &
                  !      'trkspt: sa ANTTAB(IPOFF  ) , NDIMZ   ', &
                  !                  ANTTAB(IPOFF  ) , NDIMZ
                  !write(6,'(A,4(1x,I10))') &
                  !  'trkspt: satid &
                  !  &...index_sat, ii( KANTYP + (index_sat-1) * 2 )',&
...
!-------------------------------------------------------------------------------
      ...
      IF(LPHC) THEN
        ...
        ELSE
           ...
           ENDDO
        ENDIF ! L1ST
!        write(6,'(A,1x,E20.10)')'trkspt: phc(1) ', phc(1)
...
60001 FORMAT(' TRKSPT CALLED FOR GPS MEASUREMENTS:')
