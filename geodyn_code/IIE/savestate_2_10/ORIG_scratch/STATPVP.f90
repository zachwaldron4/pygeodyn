!$STATPVP
      SUBROUTINE STATPVP(AA,II,LL,LDNPOL,LPOLAD,MJDSN,FSECN,NM,XMN,    &
     &  NINTPL,NMP,INDP,PXPOLE,RPOLE,S,XDPOLE,XTP,PXSPXP,PXSPLV,NINTP2,&
     & LCET,ISTA,LVLBI,LYARA,ISNUMB,PXSPXD,COSTHG,SINTHG,SCRTCH,UTDT,  &
     & S1,FSECT,DXDNUT,LNUTAD)
!********1*********2*********3*********4*********5*********6*********7**
...
!
      ...
      DO N=1,NM
      CALL GRHRAP(MJDSN,FSECN(N),.TRUE.,.TRUE.,AA(KEQN-1+N),SCRTCH(N,1),&
     &     AA(KTHETG-1+N),COSTHG(N),SINTHG(N),1,AA,II,UTDT(1),.TRUE.,   &
     &     XMN,TN)

! OBTAIN POLAR MOTION ROTATION MATRICES AND PARTIAL DERIVATIVE MATRICES
      CALL INPOLP(AA(KA1UT),AA(KDPSR),AA(KXPUT),AA(KYPUT),.FALSE.,      &
