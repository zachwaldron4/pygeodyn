!$EUVEAT
      SUBROUTINE EUVEAT(MJDS,FSEC,XSAT,VSAT,BFNRM1,BFNRM2,BFNRM3,       &
...
!*******************************************************************
!  ROUTINE NAME:   EUVEAT   DATE: 03/10/98      PGMR: C.M. Cox
!
!   FUNCTION - TO COMPUTE ROTATION FROM EUVE BODY-FIXED FRAME TO THE
!              GEODYN TRUE OF REFERENCE FRAME, BASED ON THE EUVE
...
!
!************ EUVEAT LOCAL VARIABLE DEFFINITIONS************************
...
!
! Note: in the EUVE case we consider the x to be along track, the y to b
...
!
! REFERENCES:
!            EUVE Dimensional Control Layout Drawings, GSFC
!
!*******************************************************************
...
!
! XAXIS ROTATION - in sun direction
!
      XTOD(1) = UNTSUN(1)
      XTOD(2) = UNTSUN(2)
      XTOD(3) = UNTSUN(3)
      ...
      SPF(3,1)= XTOD(3)
!
! YAXIS ROTATION - r cross x
!
      YTOD(1) = XSAT(2)*XTOD(3)-XSAT(3)*XTOD(2)
      YTOD(2) = XSAT(3)*XTOD(1)-XSAT(1)*XTOD(3)
      YTOD(3) = XSAT(1)*XTOD(2)-XSAT(2)*XTOD(1)
...
!2222 format(1x,'EUVEAT: i, dtemp, tdnrmn(i)',i2,4f10.6)
  800 END DO
!
  850 CONTINUE
!
