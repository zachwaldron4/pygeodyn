!$DLTDOR
      SUBROUTINE DLTDOR(AA,II,LL,TPARTL,INDSTA,INDSAT,INDSET,           &
     &   INPSTA,INPSAT,LELEVS,MTYPEX,ISEQ,MSEQ,LADD,L1RCVR,ILCORR,      &
     &   SAVEC1,SAVEC2,NM3,ILOOP,XMM,UMV,UM1,UM2)
!********1*********2*********3*********4*********5*********6*********7**
!
!
! FUNCTION:
...
!********1*********2*********3*********4*********5*********6*********7**
     ...
     &   LELEVS(3,2),INPSTA(3,2),INPSAT(3,2)
      ...
      DIMENSION INDVQL(3),INPVQL(3)
!
!***********************************************************************
! START OF EXECUTABLE CODE *********************************************
!***********************************************************************
!*
! FIRST PART OF DELTADOR  SINGLY DIFFERENCED 1 WAY RANGES
!
...
10000 END DO
      ...
      write(6,*)'                                                   '
      write(6,*)' **************  SATELLITE REPORT **************** '
      write(6,*)'                                                     '
      write(6,*)'COMPUTED S/C/STATION DIFF RANGE: ',AA(KOBSC),'SEC'
      RNAN=AA(KOBSC)*1.D+09
      write(6,*)'COMPUTED S/C/STATION DIFF RANGE : ',RNAN,'NANOSECS'

!  SECOND PART VLBI DELAY
         ...
         INPVQL(2)=INPSTA(1,2)
         INPVQL(3)=0
         CALL QVLBI(AA,II,LL,INDVQL,INPVQL,LELEVS,AA(KPMPA),AA(KSTAIN), &
     &          AA(KQUINF),AA(KVLOPT),AA(KSIGSP),II(KBSPLN),II(KSSPLN), &
     &          VLBDEL)

      WRITE(6,*)'                                                    '
      write(6,*)' COMPUTED VLBI DELAY IS: ',-VLBDEL,'SECONDS'
      VLBNAN=-VLBDEL*1.D09
      write(6,*)' COMPUTED UNCORRECTED  VLBI DEL IS:',VLBNAN,'NANOSECS'

      DELTADOR=RNAN-VLBNAN

      AA(KOBSC)=DELTADOR
      write(6,*)' COMPUTED DeltaDOR OBS IS: ',AA(KOBSC),'NANOSECS'
      WRITE(6,*)'                                                    '
      TIMEN=DBLE(MJDSBL)+FSECBL
      write(86,400)AA(KOBS),VLBNAN,TIMEN
 400  format(3F25.10)

!     TEMPORARY CODE FOR FIRST OBSERVATION

!     des=2.345212169258035D-009
!     desm=des*VLIGHT
!     write(6,*)' DBG MAG ',desm
!     desn=des*1.D09
!     AA(KOBSC)=RNAN-desn
