!$MINDST
      SUBROUTINE MINDST(ND1,NU1,T1EST,T2EST,ACOEF,T1,T2,D)
!********1*********2*********3*********4*********5*********6*********7**
...
!
  ...
  100 CONTINUE
      DO 120 J=1,2
      DO 110 I=1,3
      CALL PVAL(ND1,TM(J),ACOEF(1,1,I,J),.TRUE.,.TRUE.,                 &
...
!
  ...
  200 CONTINUE
      ...
      RETURN
 6000 FORMAT(' ')
 6001 FORMAT(' EXECUTION TERMINATING IN MINDST')
 6002 FORMAT(' DEGREE OF INPUT POLYNOMIAL: ',I3)
 6003 FORMAT(' EXCEEDS MAX ALLOWABLE: ',I3)
 6004 FORMAT(' WARNING MAX ITERATIONS ',I3,' EXCEEEDED IN MINDST')
