!$NPNCXN
      SUBROUTINE NPNCXN(MINEP,LFLAG,LINIT,IDIM1)
!********1*********2*********3*********4*********5*********6*********7**
! NPNCXN           86/12/24            8701.0    PGMR - D. ROWLANDS
!
...
!********1*********2*********3*********4*********5*********6*********7**
!
!  THE DIFF BETWEEN XJD2K AND GEODYN REF.TIME IS:
        DD0=XJD2K-TMGDN1
!
!--------------- MOON ---------------------------------
!
      IF( LMOON ) THEN
!
      IF(CLIBMN.EQ.1.D0)THEN
!
         MFSDIN=INT(FSDINP(1))
         CALL CMNLIB(MFSDIN,0.D0,XLLIB,XLLIBV)
        ALF020=XLLIB(1)
        ALFR20=XLLIBV(1)
        DEC020=XLLIB(2)
        DECR20=XLLIBV(2)
        ...
        ELSE
!
! WANT RA, DEC, THE PRIME MERIDIAN, AND THEIR RATES AT J2000
! SO CALL NLUNCN WITH D=0.0D0
!
!CC     DW = FSDINP(1) / SECDAY - 21544.5D0
!CC     D = -21544.5D0
! ABOVE IMPLIED 2430000.5   : 21544.5 = JD2000 -2430000.5
! SO  IT MUST BE MODIFIED FOR THE GEODYN FERENRENCE TIME
!
        DD0=XJD2K-TMGDN1
        DW = FSDINP(1) / SECDAY - DD0
        D=DW
         CALL NLUNCN( D,DW, ALF020, ALFR20, DEC020,DECR20,THXTGC(1,1), &
     &                THXTGC(2,1) )
         THXTGC(1,1)=MOD(THXTGC(1,1),TWOPI)+TWOPI
       ENDIF
!
!          ....changed requested by DDR 2/22/93
!          ....to change time of RA, DEC to GEODYN zero time
         ALF020 = ALF020 - FSDINP(1) * ALFR20
         DEC020 = DEC020 - FSDINP(1) * DECR20
!
!--------------- MOON ---------------------------------
      ELSE
! NOT LMOON
...
!
      IF( LFIRST ) THEN
         ...
         WRITE(6,*) 'NPNCXN: XA100, XAREST, W020 ',XA100, XAREST,       &
     &               W020
         WRITE(6,*) 'NPNCXN:THXTGC(1,1),THXTGC(2,1)',THXTGC(1,1), &
     &               THXTGC(2,1)
!
         WRITE(6,*) 'NPNCXN: MINEP, NSDNUT ', MINEP, NSDNUT
         WRITE(6,*) 'NPNCXN: ALF020        ', ALF020
         WRITE(6,*) 'NPNCXN: ALFR20        ', ALFR20
         WRITE(6,*) 'NPNCXN: DEC020        ', DEC020
         WRITE(6,*) 'NPNCXN: DECR20        ', DECR20
         WRITE(6,*) ' '
!
      ENDIF
!
