!$GEOID
      SUBROUTINE GEOID(NM,COSLAT,SINLAT,COSLON,SINLON,C,S,ICPNT,        &
     &                 ISPNT,P,COSLAM,SINLAM,EXPART,GEOIDS,PMPA,LNPNM,  &
     &                 NDIM1,NDIM2,LGEOID,LGPART,WORK,AA)
!********1*********2*********3*********4*********5*********6*********7**
! GEOID            00/00/00            0000.0    PGMR - ?
!
!
! FUNCTION:  CALCULATE GEOID HTS (AS IMPLIED BY A GRAVITY MODEL)
!            AT A SERIES OF POINTS ON THE ELLIPSOID AND/OR CALCULATE
!            THE PARTIALS OF AN ALTIMETRY MEASUREMENT TO THE GEOID
!            WRT C & S COEFFCIENTS
!
!
! I/O PARAMETERS:
...
!   ICPNT    I    A    POINTER IN C ARRAY TO ADJUSTED COEFFCIENTS
!   ISPNT    I    A    POINTER IN S ARRAY TO ADJUSTED COEFFCIENTS
!   P       I/O   A    SPACE FOR LGENDRE POLYNOMIALS
...
!   LGEOID   I    S    .TRUE.,IF CALCULATION OF GEOID HT REQUIRED
!   LGPART   I    S    .TRUE.,IF CALCULATION OF GRAVITY COEFFCIENTS
!   WORK    I/O   A    SPACE FOR ADJUSTED C&S
...
!
     ...
     &          ICPNT(1),ISPNT(1),P(1),COSLAM(NMAXP1),SINLAM(NMAXP1),   &
     &          EXPART(NP,*),                                           &
     &          GEOIDS(NM),PMPA(NDIM1,NDIM2),WORK(NADJCS),AA(1)
! %%%%%%%%% EXPART(NP,1) -> EXPART(NP,*)
!
      DATA ZERO/0.D0/,ONE/1.D0/
!     DATA GAMMAA/9.780325133D0/
!     DATA FSTAR/.005302237D0/
!     DATA F44/.000023295547/
!
!**********************************************************************
...
!
      DO 500 I=1,NM
!
! CALCULATE COSLAM AND SINLAM ARRAYS
!
      COSLAM(1)=ONE
      SINLAM(1)=ZERO
      SINLAM(2)=SINLON(I)
...
!
      ...
      SINP2=SINPSI*SINPSI
      COSP2=COSPSI*COSPSI
      GAMMA=GAMMAA*(ONE+FSTAR*SINP2-F4*SINP2*COSP2)
      R=BEG/SQRT(ONE-EGSQ*COSP2)
      AOR=AEG/R
      GMR=GM/R
      AORN1=AOR*GMR/GAMMA
!
...
!
      GEOIDS(I)=POT-(REFC20*EXPART(5,1)+REFC40*EXPART(16,1)             &
     &         +REFC60*EXPART(31,1))
      GEOIDS(I)=GEOIDS(I)
!
...
!
!       NUMBER OF PARAMETERS IS THE FIRST DIMENSION
!
!
!   PUT PARTIALS WRT C INTO PARTIAL ARRAY
!
...
!
  300 CONTINUE
!
!     NUMBER OF PARAMETERS IS THE SECOND DIMENSION
!
...
!
      DO 425 II=1,NADJS
      PMPA(I,II+ISTRTS-1)=-EXPART(ISPNT(II),2)
  425 END DO
!
  500 END DO
!
