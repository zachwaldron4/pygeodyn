!$TRKERS
      SUBROUTINE TRKERS(XSM,VSM,UI,OFFSET,FSECRA,RA,DR,AA,II,ISATID,    &
     &            LALT,NDIMRA,ATROT,LANTCT,IANTSC,L1ST,LOFFAJ,DWRKDO,   &
     &            STAINF,INDSTA,PHC,ANTTAB,LDORANT)
!********1*********2*********3*********4*********5*********6*********7**
! TRKTOP           92/02/18            9201.1    PGMR - ANDREW MARSHALL
!
!
...
!--------------------------------------------------------------------
      ...
      !      'trkers:1 MTYPE, XGPSTB      ', MTYPE, XGPSTB
      !write(6,'(A,1x,I4,1x,F6.1)') &
      !      'trkers:1 ISEQ, XANTSL(ISEQ) ', &
      !                ISEQ, XANTSL(ISEQ)
      ...
      if( XGPSTB .GT. 0.D0  .or.                     &
          ...
          !write(6,'(A,1x,I4,1x,F6.1)') &
          !      'trkers:2 MTYPE, XGPSTB            ', MTYPE, XGPSTB

          IANT=(XANTSL(ISEQ)+.001D0)

          !write(6,'(A,1x,I4,1x,F6.1, 1x, I6)') &
          !      'trkers:2 ISEQ, XANTSL(ISEQ), IANT ', &
          !                ISEQ, XANTSL(ISEQ), IANT

      endif ! xgpstb...

!      write(6,'(A,1x,I4,1x,I6, 4x, L1)') 'trkers: ISEQ, IANT, LPHC ', &
!                                                  ISEQ, IANT, LPHC
!      write(6,'(A,1x,I10)')  'trkers: ISATID ', ISATID
...
!                 'trkers: ISATID, iant, index_sat, L_sat_phc', &
...
!                 'trkers: IANT, index_ant, L_ant_phc', &
!                          IANT, index_ant, L_ant_phc
      ...
      !write(6,'(A,3x,L1)') 'trkers: LPHC ', LPHC
      ...
      IF( LPHC ) THEN
        ...
        if( NANTPT > 0 .and. L_ant_phc )then
            ...
            !write(6,'(A,4(1x,I6))') &
            !      'trkers:a IANT, IPOFF, NDIMA, NDIMZ ', &
        ...
        if( L_sat_phc )then
            ...
            !write(6,'(A,4(1x,I6))') &
            !      'trkers: sa index_sat, IPOFF, NDIMA, NDIMZ ', &
...
!      write(6,'(A,3(1x,I4))') 'trkers: MTYPE, ISEQ ', &
!                                       MTYPE, ISEQ
!      write(6,'(A,3x,L1)') 'trkers: LPHC ', LPHC
...
! COMPUTE INERTIAL OFFSET UNIT VECTOR
      DO 100 I=1,NM
        ...
        VPOS(3) = VSM(I,3)
        IF(LERS1) THEN
        CALL ERSATT(MJDSBL,FSECRA(I),XPOS,VPOS,BDFOFF(1),               &
     &           BDFOFF(2),BDFOFF(3),UNTOFF(1),UNTOFF(2),               &
     &           UNTOFF(3),DUM,1,0,.FALSE.,                             &
     &           ISATID,II(KIDATB),AA(KSABIA),AA(KSBTM1),AA(KSBTM2),    &
     &           AA(KVLOUV),II(KNSTLV),II(KSLVID),AA(KTSLOV),AA,IDUM,   &
     &           ROT,ATROT(1,1,I))
        ELSE IF(LENVS) THEN
        CALL ENVATT(MJDSBL,FSECRA(I),XPOS,VPOS,BDFOFF(1),               &
     &           BDFOFF(2),BDFOFF(3),UNTOFF(1),UNTOFF(2),               &
     &           UNTOFF(3),DUM,1,0,.FALSE.,                             &
     &           ISATID,II(KIDATB),AA(KSABIA),AA(KSBTM1),AA(KSBTM2),    &
     &           AA(KVLOUV),II(KNSTLV),II(KSLVID),AA(KTSLOV),AA,IDUM,   &
...
!-------------------------------------------------------------------------------
      ...
      IF( LPHC ) THEN
          ...
          if( i == 1 )then
              ...
              !      'trkers: XV, YV, ZV ', XV, YV, ZV
              !write(6,'(A,2(1x,I10))')  'trkers: I, IMQP ', i, imqp

              !if( NANTPT > 0 .and. L_ant_phc )then
              !    write(6,'(A,3(1x,I10))') &
              !          'trkers: iant, kantps(iant), ipoff ', &
              ...
              if( L_sat_phc )then
                  ...
                  !      'trkers: sa ANTTAB(IPOFF+3) , NDIMA   ', &
                  !                  ANTTAB(IPOFF+3) , NDIMA
                  !write(6,'(A,1x,E15.7,1x,I10)') &
                  !      'trkers: sa ANTTAB(IPOFF  ) , NDIMZ   ', &
                  !                  ANTTAB(IPOFF  ) , NDIMZ
...
!-------------------------------------------------------------------------------
      ...
      IF(LPHC) THEN
        ...
        ELSE
           ...
           ENDDO
        ENDIF ! L1ST
!        write(6,'(A,1x,E20.10)')'trkers: phc(1) ', phc(1)
...
60001 FORMAT(' TRKERS CALLED FOR GPS MEASUREMENTS:')
