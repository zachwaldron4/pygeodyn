!$SUBTRK
      SUBROUTINE SUBTRK(XYZ,THETAG,XY2,ZT,RT,SATLAT,SATLON,SATH,        &
     &                  NM)
!********1*********2*********3*********4*********5*********6*********7**
...
!   ------  ---  ---   ------------------------------------------------
!   XYZ      I         XYZ S/C INERTIAL POSITIONS
!   THETAG   I         RIGHT ASCENSIONS OF GREENWICH
...
!********1*********2*********3*********4*********5*********6*********7**
     ...
     &       NSTAIN,NXCNIG
      ...
      DIMENSION XYZ(MINTIM,3),THETAG(NM),XY2(NM),ZT(NM),SATLAT(NM),     &
...
!
 ...
 2800 END DO
          RADDEG=ONE/DEGRAD
      DO 3800 N=1,NM
      SATLON(N)=(SATLON(N)-THETAG(N))*RADDEG
 3800 END DO
      DO 4800 N=1,NM
