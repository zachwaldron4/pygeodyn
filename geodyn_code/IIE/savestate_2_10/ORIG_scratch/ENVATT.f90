!$ENVATT
      SUBROUTINE ENVATT(MJDS,FSEC,XSAT,VSAT,BFNRM1,BFNRM2,BFNRM3,       &
     ...
     &                  ATROT)
!*******************************************************************
!  ROUTINE NAME:   ENVATT   DATE: OCT 2008      PGMR: A. SIBTHROP
!                                                     CHANGXUE DENG
!
...
!
     ...
     &          TEMP(3,3),SBF(3,3),SBFP(3,3),AA(1),QAT(4),TEMP1(3,3)
      ...
      dimension y1(3),y2(3),y3(3)
      dimension z1(3),z2(3),z3(3)
      dimension h(3), rot_quat(0:3)
!     PARAMETER(Cx = 0.1672D0*DEGRAD)
!     PARAMETER(Cy =-0.0501D0*DEGRAD)
!     PARAMETER(Cz = 3.9130D0*DEGRAD)
      PARAMETER(Cx = 2.9181905093345187D-3)
      PARAMETER(Cy =-8.744099552491591D-4)
      PARAMETER(Cz = 0.06829473363053812D0)
!
...
! argument of latidue = arctan(z/tmp), where
!                 tmp=-x*W_y+y*W_x
!                 W_y = h_y/h, and W_x = h_x/h
! (hx,hy,hz) is the angular momentum vector
! First compute the angular momentum
      h(1) = XSAT(2)*VSAT(3) - XSAT(3)*VSAT(2)
      h(2) = XSAT(3)*VSAT(1) - XSAT(1)*VSAT(3)
      h(3) = XSAT(1)*VSAT(2) - XSAT(2)*VSAT(1)
      h_mag = SQRT(h(1)*h(1)+h(2)*h(2)+h(3)*h(3))
      h(1) = h(1)/h_mag
      h(2) = h(2)/h_mag
!     h(3) = h(3)/h_mah
      dum = -XSAT(1)*h(2) + XSAT(2)*h(1)
      Ulat = ATAN2(XSAT(3),dum)
      IF(Ulat .LT. 0.D0) Ulat = Ulat + TWOPI
!
...
! Compute pitch, roll, and yaw angles
      pitch = Cx*SIN(2.D0*Ulat)
      roll  = Cy*SIN(Ulat)
      dum   = Cz*COS(Ulat)
      yaw   = Cz*COS(Ulat)*(1.D0-dum*dum/3.D0)
! Get negative xtod and ytod
      xtod(1) = -xtod(1)
      xtod(2) = -xtod(2)
      xtod(3) = -xtod(3)
      ytod(1) = -ytod(1)
...
! Roll
      CALL CONVERT_FROM_ANGLE_AXIS(roll,ytod,rot_quat)
      CALL QUATERNION_ROTATE(xtod,rot_quat,x1)
      CALL QUATERNION_ROTATE(ytod,rot_quat,y1)
      CALL QUATERNION_ROTATE(ztod,rot_quat,z1)
! Pitch
      CALL CONVERT_FROM_ANGLE_AXIS(pitch,x1,rot_quat)
      CALL QUATERNION_ROTATE(x1,rot_quat,x2)
      CALL QUATERNION_ROTATE(y1,rot_quat,y2)
      CALL QUATERNION_ROTATE(z1,rot_quat,z2)
! Yaw
      CALL CONVERT_FROM_ANGLE_AXIS(yaw,z2,rot_quat)
      CALL QUATERNION_ROTATE(x2,rot_quat,x3)
      CALL QUATERNION_ROTATE(y2,rot_quat,y3)
      CALL QUATERNION_ROTATE(z2,rot_quat,z3)
!
! Fill TOTROT
      DO i=1,3
        TOTROT(i,1) = X3(i)
...
!....PERTINENT ANGLES
     ...
     &             VSAT(3),RSATID
      ENDIF
!
! Refill TOTROT with x3, which will be used in UCL_ENVI routine if called from F
! Otherwise leave TOTROT as it is if called from TRKERS
      IF(LFORCE) THEN
        DO i=1,3
          TOTROT(i,1) = x3(i)
          TOTROT(i,2) = y3(i)
