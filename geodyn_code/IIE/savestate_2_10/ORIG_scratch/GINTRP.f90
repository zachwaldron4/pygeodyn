!$GINTRP
      SUBROUTINE GINTRP(IWINDO,DMIN,H,PHIS,DLAW,DDFI,DDLA,NPHI,NDLA,    &
...
!
      ...
      REARTH=6374000.D0
!
! ABOUT THE REARTH VARIABLE; IT DOES NOT NEED TO BE CHANGED FOR PLANETARY RUNS
! IT IS USED TOGETHER WITH THE INPUT ARGUMENT, DMIN TO GET A TOLERANCE ANGLE
! FOR HOW CLOSE THE POINT TO BE INTERPOLATED IS TO THE GRID BOUNDARY. ALL CALLS
! TO GINTRP IN GEODYN ARE MASE WITH DMIN=5.D0 (KM), SO THE TOLERANCE ANGLE IS
! SET TO 0.05 DEGREES
!
      IF(IWINDO.LT.2) IWINDO=2
      IF(IWINDO.GT.IDIM1) IWINDO=IDIM1
      ILIM=DMIN*1000.D0*RHO/(REARTH*DDFI)
      JLIM=DMIN*1000.D0*RHO/(REARTH*DDLA*COS((PHIS+DDFI*NPHI/2.D0)/RHO))
