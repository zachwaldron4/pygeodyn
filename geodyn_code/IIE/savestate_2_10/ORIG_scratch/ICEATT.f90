...
!
!************ ICEATT LOCAL VARIABLE DEFFINITIONS************************
...
! FOR NOW ASSUME Ysc POINTING VELOCITY WHEN B' > 32 DEG.
!               -Ysc POINTING VELOCITY WHEN B' < -32 DEG.
!
!      if(.not.lforce) print *, ' iceatt: betap: ',betap/degrad
      IF(BETAP/DEGRAD < -33.0D0) THEN
       SBF(1,1) = ZERO
       SBF(1,2) = ONE
       SBF(1,3) = ZERO
       SBF(2,1) = ZERO
       ...
       SBF(3,3) = ZERO
       YAWIND = 1.0D0
      ELSEIF(BETAP/DEGRAD > 33.0D0) THEN
       SBF(1,1) = ZERO
       SBF(1,2) = -ONE
       SBF(1,3) = ZERO
       SBF(2,1) = ZERO
       ...
       SBF(3,3) = ZERO
       YAWIND = 2.0D0
      ELSEIF( BETAP/DEGRAD >= -33.0D0 .and. BETAP/DEGRAD <= 0.0D0) THEN
       SBF(1,1) = ZERO
       SBF(1,2) = ZERO
       SBF(1,3) = ONE
       SBF(2,1) = ZERO
       ...
       SBF(3,3) = ZERO
       YAWIND = 3.0D0
      ELSEIF( BETAP/DEGRAD > 0.0D0 .and. BETAP/DEGRAD <= 33.0D0) THEN
       SBF(1,1) = ZERO
       SBF(1,2) = ZERO
       SBF(1,3) = -ONE
       SBF(2,1) = ZERO
       ...
       YAWIND = 4.0D0
      ELSE
       WRITE(*,*) 'ABNORMAL TERM. IN ICEATT: NO MATCH FOR YAW REGIME'
       STOP 69
      ENDIF
!
! COMPUTE GCS TO SPF FROM GCS_2_SCS and SCS_2_SPF AND RESET INTO SBF
!
      DO I = 1,3
       TEMP2(I,1) = SBF(I,3)
       TEMP2(I,2) = -SBF(I,2)
...
!
  ...
  600 END DO
! sbl
!      print *,'iceatt: 1,1-3 : ',totrot(1,1),totrot(1,2),totrot(1,3)
!      print *,'iceatt: 2,1-3 : ',totrot(2,1),totrot(2,2),totrot(2,3)
!      print *,'iceatt: 3,1-3 : ',totrot(3,1),totrot(3,2),totrot(3,3)
! sbl
...
!
! *** SOLAR ARRAY ANGLE MODEL ASSUMES ROTATE ABOUT +Y IN SCS AND 0 DEG.
!     WHEN POINTING TOWARD -Z IN SCS.  ASSUME OPTIMAL POINTING TO SUN,
!     BUT KEEPING PERPENDICULAR TO Y SCS AXIS.
!     THE Y SCS AXIS IN TOD IS THE 2ND COLUMN OF TOTROT

! COMPUTE S/C TO SUN VECTOR
      VEC1(1) = BDTRUE(1,8) + ZTOD(1)
      VEC1(2) = BDTRUE(2,8) + ZTOD(2)
      VEC1(3) = BDTRUE(3,8) + ZTOD(3)
      VMAG = SQRT(VEC1(1)**2+VEC1(2)**2+VEC1(3)**2)
      VEC1(1) = VEC1(1)/VMAG
      VEC1(2) = VEC1(2)/VMAG
      VEC1(3) = VEC1(3)/VMAG

! FIND VECTOR IN Solar Array (SA) PLANE: Yscsintod X sctosun vector
      VEC2(1) = TOTROT(2,2)*VEC1(3)-TOTROT(3,2)*VEC1(2)
      VEC2(2) = TOTROT(3,2)*VEC1(1)-TOTROT(1,2)*VEC1(3)
      VEC2(3) = TOTROT(1,2)*VEC1(2)-TOTROT(2,2)*VEC1(1)

! COMPUTE SA ANGLE WITH DEFINITIONS DESCRIBED IN COMMENTS ABOVE
! ... SA angle reference is the -Z SCS axis
      VEC1(1) = -TOTROT(1,3)
      VEC1(2) = -TOTROT(2,3)
      VEC1(3) = -TOTROT(3,3)

      VEC1M=SQRT(VEC1(1)*VEC1(1)+VEC1(2)*VEC1(2)+VEC1(3)*VEC1(3))
      VEC2M=SQRT(VEC2(1)*VEC2(1)+VEC2(2)*VEC2(2)+VEC2(3)*VEC2(3))
      DOT=VEC2(1)*VEC1(1) + VEC2(2)*VEC1(2) + VEC2(3)*VEC1(3)
      ARGQ=DOT/(VEC1M*VEC2M)
      IF(ARGQ.GT.1.D0) ARGQ=0.999999999999999D0
      IF(ARGQ.LT.-1.D0) ARGQ=-0.999999999999999D0
      SGAMMA=ACOS(ARGQ) - PI/TWO
     ...
     &  (TIME.GE.TIMBI1(I)).AND.(TIME.LE.TIMBI2(I)))SABTMP=SABIAS(I)
      ...
      SGAMMA = SGAMMA + SABTMP

! COMPUTE ROTATION FOR SOLAR ARRAY (To rotate from the solar array frame
! the SCS frame, rotate about the SCS -y axis by the sum of
! the SA angle and PI/2.  the x axis in the SA frame is the sun tracking

      SGAMRT = SGAMMA + PI/TWO
      COSSGR = COS(SGAMRT)
      SINSGR = SIN(SGAMRT)
      ...
      SAROT(3,3) = COSSGR
!
! COMPUTE SOLAR ARRAY FRAME TO GCS FROM SOLAR_2_SCS and SCS_2_GCS
!
       TEMP2(1,1) = SAROT(3,1)
       TEMP2(1,2) = SAROT(3,2)
       TEMP2(1,3) = SAROT(3,3)
...
! ROTATE BODY-FIXED LOUVER ACCELERATION VECTOR IF NECESSARY
 ...
 1000  CONTINUE
         ENDIF
!
! OUTPUT ICESat TELEM FILE INFORMATION
...
!....PERTINENT ANGLES
         ...
         CALL QATCMP(XSAT,VSAT,QAT,RTMP,PTMP,YTMP)
         WRITE(97) DIYMD,DIHM,SEC,YAWIND,BETAP/DEGRAD,SOMEGA/DEGRAD, &
     &             YTMP,SGAMMA/DEGRAD,CM999,QAT(1),QAT(2),QAT(3), &
