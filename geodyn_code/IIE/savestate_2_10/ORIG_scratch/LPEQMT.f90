...
!
      ...
      SAVE
!
      PARAMETER (RAD= 0.017453292519943D0)
...
!     ---------------------------------------------------------
      ...
      DO 1 N=1,4
         PH = PHC(N) + TD*DPD(N)
         SHPN(N) = RAD*MOD(PH,36.D1)
    1 END DO
!
!     Assemble long-period tide potential from 15 CTE terms > 1 mm.
!     Nodal term is included but not the constant term.
!     -------------------------------------------------
      ZLP = 2.79D0 * COS( SHPN(4) )                                    &
     &     -0.49D0 * COS( SHPN(2) - PSOL )                             &
     &     -3.10D0 * COS( 2.D0*SHPN(2) )
      PH = SHPN(1)
      ZLP = ZLP - 0.67D0 * COS( PH - 2.D0*SHPN(2) + SHPN(3) )          &
     &          -(3.52D0 - 0.46D0*COS(SHPN(4))) * COS( PH - SHPN(3) )
      PH = PH + SHPN(1)
      ZLP = ZLP - 6.66D0 * COS( PH )                                   &
     &          - 2.76D0 * COS( PH + SHPN(4) )                         &
     &          - 0.26D0 * COS( PH + 2.D0*SHPN(4) )                    &
     &          - 0.58D0 * COS( PH - 2.D0*SHPN(2) )                    &
     &          - 0.29D0 * COS( PH - 2.D0*SHPN(3) )
      PH = PH + SHPN(1)
      ZLP = ZLP - 1.27D0 * COS( PH - SHPN(3) )                         &
     &          - 0.53D0 * COS( PH - SHPN(3) + SHPN(4) )               &
     &          - 0.24D0 * COS( PH - 2.D0*SHPN(2) + SHPN(3) )
!
