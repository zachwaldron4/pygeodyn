!$LITEUP
      SUBROUTINE LITEUP(X1,X2,X2PC,R,WORK,WORK2,WORK3,WORK4,MJDS1,FSEC1,&
     &         FSEC2,SIGNL,DELCET,RSS,GRLLT,CBSTAT,NM,                  &
     &         SCRTM,TPMEAS,LTPMES,ISATID,AA,II,LL,LSTST)
!********1*********2*********3*********4*********5*********6*********7**
...
!                      POINT #2
!   X2PC     O    A    INERTIAL TRUE OF REF POSITION VECTORS FOR
!                      POINT #2 ; PLANET CENTERED IN THE CASE OF
!                      AN INTERPLANETARY RUN
...
!   LSTST    I    S    TRUE IF FOR A SAT SAT LINK
!
! COMMENTS:
...
!
     ...
     &   SCRTM(NM,90),TPMEAS(MINTIM,2),LTPMES(MINTIM,3),X2PC(MINTIM,3)
...
!      DATA GRLFC3/.29587322D-10/  NOW IN CB CRELAT
      ...
      KOUNT=0
      LCOUNT=.FALSE.
      IF(ITBDGM.NE.ICBDGM.AND..NOT.LSTST) GO TO 10000
! TRACKING BODY IS SAME AS CENTER OF INTEGRATION
 1000 CONTINUE
      ...
      write(6,*)'DBGGG ',MAPSAT(INTOSP,1),MAPSAT(INTOSP,2),    &
     & MAPSAT(INTOSP,4),KNSTEPS,NEQNI,IRET
      ENDIF
      ENDIF
...
! CASE. IN THIS CASE ORBIT WAS EVALUATED TOO EARLY.
      DO 10300 N=1,NM
      FSEC2(N)=FSEC2(N)-FACT1*X2PC(N,1)-FACT2*X2PC(N,2)                 &
     &                 -FACT3*X2PC(N,3)
10300 END DO
!******8DEBUG****************
!     PRINT*,' FSEC@2 ',FSEC2(1)
!     PRINT*,' FACT ',FACT1,FACT2,FACT3
!     PRINT*,' X2   ',X2PC(1,1),X2PC(1,2),X2PC(1,3)
!***************************
!
...
!    1       MINTIM,NM,.TRUE.,.TRUE.,.FALSE.,.TRUE.,AA)
      IF (.NOT. LASTSN) THEN
        IF (.NOT. (LXEPHM.AND.ICBDGM.GT.11)) THEN
          CALL BARYC(MJDS1,FSEC2,X2,X2(1,1,1,2),MINTIM,IGRP,IORCB,      &
     &               IORMO,IORSUN,IOREM,COEF,COEFMO,COEFS,COEFEM,       &
     ...
     &               SCRTM(1,46),SCRTM(1,47),SCRTM(1,48),AA,II)
        ELSE
          CALL BARYCX(MJDS1,FSEC2,X2,X2(1,1,1,2),MINTIM,IGRP,IORCB,     &
     &               IORMO,IORSUN,IOREM,COEFX,COEFMO,COEFS,COEFEM,      &
     &               NM,NDIM1,NDIM2,.TRUE.,.FALSE.,.TRUE.,AA(KPRCBD),   &
     &               AA(KPRTBD),SCRTM(1,1),SCRTM(1,7),SCRTM(1,10),      &
     &               SCRTM(1,16),SCRTM(1,31),SCRTM(1,16),SCRTM(1,31),   &
     &               SCRTM(1,16),SCRTM(1,31),                           &
     &               SCRTM(1,46),SCRTM(1,47),SCRTM(1,48),AA,II)
        END IF
      ELSE
        CALL BARYCA(MJDS1,FSEC2,X2,X2(1,1,1,2),MINTIM,IGRP,IORCB,       &
     &             IORMO,IORSUN,IOREM,COEFX,COEFMO,COEFS,COEFEM,        &
     &             NM,NDIM1,NDIM2,.TRUE.,.FALSE.,.TRUE.,AA(KPRCBD),     &
     &             AA(KPRTBD),SCRTM(1,1),SCRTM(1,7),SCRTM(1,10),        &
     &             SCRTM(1,16),SCRTM(1,31),SCRTM(1,16),SCRTM(1,31),     &
