      SUBROUTINE GETRADEC (NM,DPOR,RA,DEC,IAST)
!-------------------------------------------------------------------------------
! Compute RA and DEC from R at each of NM epochs
!
! R   = input current 3 x 3 reference frame axis matrix
! NM  = Number of epochs
! DPOR = Input array of dynamic orientation parameters (AA(KDPOR))
! RA  = output NM length vector of RA
! DEC = output NM length vector of DEC
!
...
!-------------------------------------------------------------------------------
     ...
     &       NSTAIN,NXCNIG
      ...
      DIMENSION RA(NM),DEC(NM),DPOR(MINTIM,NDPOR,*)
      FHPI=0.5D0*ACOS(-1.D0)
      DO I=1,NM
         R13   = DPOR(I,10,IAST)
         R23   = DPOR(I,11,IAST)
         R33   = DPOR(I,12,IAST)
