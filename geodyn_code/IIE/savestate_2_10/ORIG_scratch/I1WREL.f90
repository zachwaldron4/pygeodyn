      SUBROUTINE I1WREL(MJDSBL,NM,ISAT,TC,TCP,T2S,T2E,REL,XSM,VSM,      &
     &                  ISATID,  AA,II,LL)
...
!  GET 2ND DERIV OF CHEB POLYNOMIALS FOR CENTRAL BODY
      ...
      TWOT=2.D0*TCEN
      DO 70 IDEG=3,IDEGC
      CCHEBA(IDEG)=4.D0*CCHEBV(IDEG-1)+TWOT*CCHEBA(IDEG-1)              &
     &            -CCHEBA(IDEG-2)
   70 END DO
      DO 90 IX=1,3
      CBODYX(IX)=0.D0
      NDEG=IDEGC-2
      DO 80 IDEG=1,NDEG
      CBODYX(IX)=CBODYX(IX)+EPCEN(IDEGC+1-IDEG,IX)*CCHEBA(IDEGC+1-IDEG)
   80 END DO
      CBODYX(IX)=CBODYX(IX)*BMACEN*BMACEN
   90 END DO
