!COWELA
      SUBROUTINE COWELA(LORBIT,LNDRAG,IBACK,IBACKV,                     &
     ...
     &   XRDOT,SUMXR,XMNRT,ISQ,XR,XRP,NAST,GM2,IDA1,IDA2)
!********1*********2*********3*********4*********5*********6*********7**
...
!     &   PXDTA3(NEQNA,N3,NSTEPV)
      DIMENSION XPA(3,2,2),XA(3,2,2),SUMXA(N3,2,NH,NAST,1),             &
     &   XDDOTA(N3,NSTEPS,NAST,1),PXA(NEQNA3,2,2),PXA3(NEQNA,N3,2,2),   &
     &   SUMPXA(NEQNA3,2,NHV,NAST,1),PXDDTA(NEQNA3,NSTEPV,NAST,1),      &
     &   PXDTA3(NEQNA,N3,NSTEPV,NAST,1)
      DIMENSION XDDCMP(3),XDDCM2(3),XDDSUN(3),VMATA(3,6,2),VMATX(3,6)
      DIMENSION XDDDMP(3)
!... Begin TJS...
!...  DIMENSION XRP(12,2),XR(12,2),SUMXR(12,2,NH,1),XRDOT(12,NSTEPS,1)
      DIMENSION XRP(NDPOR,2,2),XR(NDPOR,2,2),SUMXR(NDPOR,2,NH,NAST,1)
      DIMENSION XRDOT(NDPOR,NSTEPS,NAST,1)
!... End TJS...
      DIMENSION TRQP(3),TRQS(3),XDDP(3)
      DIMENSION XMNRT(3)
      DIMENSION XDUM(3,4)
...
!
      DO 1301 I=1,NMOVES
      SUMPXA(I,1,1,1,ISQ)=SUMPXA(I+NSHFTS,1,1,1,ISQ)
 1301 END DO
      IF(NAST.EQ.2) THEN
        DO 1302 I=1,NMOVES
        SUMPXA(I,1,1,NAST,ISQ)=SUMPXA(I+NSHFTS,1,1,NAST,ISQ)
 1302   END DO
      ENDIF
!
!
      DO 1303 I=1,NMOVEA
      PXDDTA(I,1,1,ISQ)=PXDDTA(I+NSHFTA,1,1,ISQ)
 1303 END DO
      IF(NAST.EQ.2) THEN
        DO 1304 I=1,NMOVEA
        PXDDTA(I,1,NAST,ISQ)=PXDDTA(I+NSHFTA,1,NAST,ISQ)
 1304   END DO
      ENDIF
!
...
!
      ...
      SUMXA(I,1,1,1,ISQ)=SUMXA(I+NSHFTS,1,1,1,ISQ)
 1700 END DO
      IF(NAST.EQ.2) THEN
        DO 1701 I=1,NMOVES
        SUMXA(I,1,1,2,ISQ)=SUMXA(I+NSHFTS,1,1,2,ISQ)
 1701   END DO
      ENDIF
!
!
      ...
      XDDOTA(I,1,1,ISQ)=XDDOTA(I+NSHFTA,1,1,ISQ)
      LTPXAT(I,1)=LTPXAT(I+NSHFTA,1)
 1800 END DO
      IF(NAST.EQ.2) THEN
        DO 1801 I=1,NMOVEA
        XDDOTA(I,1,2,ISQ)=XDDOTA(I+NSHFTA,1,2,ISQ)
 1801   END DO
      ENDIF
      DO 1805 I=1,NMVTPX
 ...
 1805 END DO
      IF(LINTAX) THEN
         DO I=1,NMVAXS
           SUMXR(I,1,1,1,ISQ)=SUMXR(I+NSHFSX,1,1,1,ISQ)
         ENDDO
         DO I=1,NMVAXA
           XRDOT(I,1,1,ISQ)=XRDOT(I+NSHFAX,1,1,ISQ)
         ENDDO
         IF(NAST.EQ.2) THEN
           DO I=1,NMVAXS
             SUMXR(I,1,1,NAST,ISQ)=SUMXR(I+NSHFSX,1,1,NAST,ISQ)
           ENDDO
           DO I=1,NMVAXA
             XRDOT(I,1,NAST,ISQ)=XRDOT(I+NSHFAX,1,NAST,ISQ)
           ENDDO
         ENDIF
      ENDIF
!
...
!!!         STOP
     ...
     &            TOPXAT(1,IORBAK),LTPXAT(1,IORBAK),PXDDAC,NEQNAC,      &
     &            IPTFBS,                                               &
     ...
     &            XPNC,XP,DSROT,XDDRC(1,IORBAK),LQ1,IPSBRN,LQ2,XDDBRN,  &
     &            AA,II,LL)
          IF(LQ1) THEN
            LSBURN=.TRUE.
            XDDBRS(1)=XDDBRN(1)
...
!
! IF(NAST=1) THERE WILL BE DUPLICATE CALCULATIONS
!
      DO 2100 J=1,N3
      SUM=SUMX(J,1,IBACK1)
      SUMA=SUMXA(J,1,IBACK1,1,ISQ)
      SUMB=SUMXA(J,1,IBACK1,NAST,ISQ)
      DO 2000 I=1,IOL2
      K=IORBAK-I
      SUM=SUM+CPP(I)*XDDOT(J,K)
      SUMA=SUMA+CPP(I)*XDDOTA(J,K,1,ISQ)
      SUMB=SUMB+CPP(I)*XDDOTA(J,K,NAST,ISQ)
 2000 END DO
      XP(J,1)=SUM*H2
      XPA(J,1,1)=SUMA*H2
      XPA(J,1,2)=SUMB*H2
...
! PREDICT VELOCITY
      DO 2300 J=1,N3
      SUM=SUMX(J,2,IBACK1)
      SUMA=SUMXA(J,2,IBACK1,1,ISQ)
      SUMB=SUMXA(J,2,IBACK1,NAST,ISQ)
      DO 2200 I=1,IOL1
      K=IORBAK-I
      SUM=SUM+CPV(I)*XDDOT(J,K)
      SUMA=SUMA+CPV(I)*XDDOTA(J,K,1,ISQ)
      SUMB=SUMB+CPV(I)*XDDOTA(J,K,NAST,ISQ)
 2200 END DO
      XP(J,2)=SUM*H
      XPA(J,2,1)=SUMA*H
      XPA(J,2,2)=SUMB*H
 2300 END DO
 2400 CONTINUE
      CALL DSTATE(MJDSEC,FSEC,IORDER,IOL1,IOL2,H,H2,SUMX(1,1,IBACK1),   &
     &            XDDOT(1,IORBAK-IOL1),XP,                              &
     &            1,3,CPP,CPV,CCP,                                      &
     &            NXSTAT,TXSTAT,                                        &
     &            DELXST,IPXST,HREMAL,LXSTAT,JPXST,DSROT)
...
!...  DO 2430 J=1,12
      DO 2430 J=1,NDPOR
!... End TJS...
      SUMR=SUMXR(J,2,IBACK1,1,ISQ)
      SUMR2=SUMXR(J,2,IBACK1,NAST,ISQ)
      DO 2420 I=1,IOL1
      K=IORBAK-I
      SUMR=SUMR+CPV(I)*XRDOT(J,K,1,ISQ)
      SUMR2=SUMR2+CPV(I)*XRDOT(J,K,NAST,ISQ)
 2420 END DO
      XRP(J,2,1)=SUMR*H
      XRP(J,2,2)=SUMR2*H
 2430 END DO
      CALL LOADAX(XRP(4,2,1),XRP(4,2,2))
 2450 CONTINUE
...
! EVALUATE ACCELERATION
     ...
     &            XDDOTA(1,IORBAK,2,ISQ),TRQP,TRQS,VMATA(1,1,2),0,      &
     &            AA,II,INTCRD,MJDSEC,FSEC,PXDTA3(1,1,IFILL,1,ISQ),     &
     &            PXDTA3(1,1,IFILL,2,ISQ),NEQNA,NEQNA)
     ...
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),1,XDDP,XPA(1,1,NAST))
      CALL F123(MJDSEC,FSEC,XP,XDDOT(1,IORBAK),PXDDT3(1,1,IFILL),NSAT,  &
     &  NEQN,                                                           &
     &  INTCRD,XDDTMC,TOPXAT(1,IORBAK),LTPXAT(1,IORBAK),AA,II,LL,ISATID,&
     &  LFHIRT,LSURFF,XDDTNC(1,IORBNC),XDDTAC(1,1,1,IORBNC),            &
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),2,XDDP,XPA(1,1,NAST))
      CALL F123(MJDSEC,FSEC,XP,XDDOT(1,IORBAK),PXDDT3(1,1,IFILL),NSAT,  &
     &  NEQN,                                                           &
     &  INTCRD,XDDTMC,TOPXAT(1,IORBAK),LTPXAT(1,IORBAK),AA,II,LL,ISATID,&
     &  LFHIRT,LSURFF,XDDTNC(1,IORBNC),XDDTAC(1,1,1,IORBNC),            &
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),3,XDDP,XPA(1,1,NAST))
!
!
          IF(NAST.EQ.2) THEN
      CALL F123(MJDSEC,FSEC,XP,XDDOT(1,IORBAK),PXDDT3(1,1,IFILL),NSAT,  &
     ...
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),4,XDDP,XPA(1,1,NAST))
      CALL F123(MJDSEC,FSEC,XP,XDDOT(1,IORBAK),PXDDT3(1,1,IFILL),NSAT,  &
     &  NEQN,                                                           &
     &  INTCRD,XDDTMC,TOPXAT(1,IORBAK),LTPXAT(1,IORBAK),AA,II,LL,ISATID,&
     &  LFHIRT,LSURFF,XDDTNC(1,IORBNC),XDDTAC(1,1,1,IORBNC),            &
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),5,XDDP,XPA(1,1,NAST))
      CALL F123(MJDSEC,FSEC,XP,XDDOT(1,IORBAK),PXDDT3(1,1,IFILL),NSAT,  &
     &  NEQN,                                                           &
     &  INTCRD,XDDTMC,TOPXAT(1,IORBAK),LTPXAT(1,IORBAK),AA,II,LL,ISATID,&
     &  LFHIRT,LSURFF,XDDTNC(1,IORBNC),XDDTAC(1,1,1,IORBNC),            &
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),6,XDDP,XPA(1,1,NAST))
          ENDIF
!
!
      HQ=H
     ...
     &  PXDDAC(1,1,IORBNC),NEQNAC,IPTFBS,NPRMAC,.FALSE.,LATOT,ONE,      &
     &  DYNTIM,NPERDY,LACCELS,EXACCT,EXACIN,EXACOB,ACCTIM,ACCPER,NPERAC,&
     &  II(KIACCP),AA(KPACCL),NRAT,XDDRC(1,IORBAK),XDDP,XPA(1,1,NAST),  &
     &  LQ1,IPSBRN,LQ2,XDDBRN,HQ)
        IF(LQ1) THEN
          LSBURN=.TRUE.
          XDDBRS(1)=XDDBRN(1)
          XDDBRS(2)=XDDBRN(2)
     ...
     &          PXDTA3(1,1,IFILL,1,ISQ),XDDOTA(1,IORBAK,1,ISQ),XDDSUN,  &
     &          XDDCMP,XDDOT(1,IORBAK),XP,XMNRT,XRP(4,2,1),XRP(1,2,1),  &
     &          XRDOT(4,IORBAK,1,ISQ),XRDOT(1,IORBAK,1,ISQ),.FALSE.,    &
     &          XDDTMC,.FALSE.,INTCRD,ONE,                              &
!... Begin TJS...
...
! d(orientation)/d(inertia)       => XRP(157,2) => length  72 /
     &        XRP(13,2,1),XRDOT(13,IORBAK,1,ISQ),XRP(157,2,1),        &
     &        XRDOT(157,IORBAK,1,ISQ),XDDP,1,TRQP,PXDDT3(1,1,IFILL),NEQN)
!... End TJS...
!
!
          IF(NAST.EQ.2) THEN
...
!
      CALL FAST2(XPA(1,1,2),XDDOTA(1,IORBAK,2,ISQ),XDDCM2,VMATA(1,1,2))
      CALL FAST(MJDSEC,FSEC,AA,II,LL,NEQNA,XPA,VMATA,                   &
     &          PXDTA3(1,1,IFILL,1,ISQ),XDDOTA(1,IORBAK,1,ISQ),XDDSUN,  &
     &          XDDCMP,XDDOT(1,IORBAK),XP,XMNRT,XRP(4,2,NAST),          &
     &          XRP(1,2,NAST),                                          &
     &          XRDOT(4,IORBAK,NAST,ISQ),XRDOT(1,IORBAK,NAST,ISQ),      &
     &          .FALSE.,XDDTMC,.FALSE.,INTCRD,ONE,                      &
!... Begin TJS...
! omega                           => XRP(  1,2) => length   3 \
! axis                            => XRP(  4,2) => length   9  \
!                                                               | Total
...
! d(orientation)/d(orientation_0) => XRP( 13,2) => length 144  /
! d(orientation)/d(inertia)       => XRP(157,2) => length  72 /
     &          XRP(13,2,NAST),XRDOT(13,IORBAK,NAST,ISQ),               &
     &          XRP(157,2,NAST),XRDOT(157,IORBAK,NAST,ISQ),XDDP,2,TRQS, &
...
! CORRECT POSITION AND VELOCITY
      ...
      SUMA=SUMA+CCP(I)*XDDOTA(J,K,1,ISQ)
      SUMB=SUMB+CCP(I)*XDDOTA(J,K,NAST,ISQ)
 2600 END DO
      X(J,1)=SUM*H2
      XA(J,1,1)=SUMA*H2
      XA(J,1,NAST)=SUMB*H2
      ...
      SUMA=SUMA+CCV(I)*XDDOTA(J,K,1,ISQ)
      SUMB=SUMB+CCV(I)*XDDOTA(J,K,NAST,ISQ)
 2700 END DO
      X(J,2)=SUM*H
      XA(J,2,1)=SUMA*H
      XA(J,2,NAST)=SUMB*H
...
!... End TJS...
      ...
      DO 2810 I=1,IOL1
      K=IORBAK-I+1
      SUMR=SUMR+CCV(I)*XRDOT(J,K,1,ISQ)
      SUMR2=SUMR2+CCV(I)*XRDOT(J,K,NAST,ISQ)
 2810 CONTINUE
      XR(J,2,1)=SUMR*H
      XR(J,2,NAST)=SUMR2*H
!... Begin TJS...
      PLOR(J,1)=XR(J,2,1)
      PLOR(J,2)=XR(J,2,NAST)
!... End TJS...
 2820 CONTINUE
 2830 CONTINUE
!
...
! NOT CONVERGED PERFORM ADDITIONAL CORRECTION
      ...
      XPA(J,1,1)=XA(J,1,1)
      XPA(J,2,1)=XA(J,2,1)
      XPA(J,1,NAST)=XA(J,1,NAST)
      XPA(J,2,NAST)=XA(J,2,NAST)
 2900 END DO
...
!...  DO 2901 J=1,12
      DO 2901 J=1,NDPOR
      PLOR(J,1)=XR(J,2,1)
      PLOR(J,NAST)=XR(J,2,NAST)
!... End TJS...
      XRP(J,2,1)=XR(J,2,1)
      XRP(J,2,NAST)=XR(J,2,NAST)
 2901 END DO
      GO TO 2500
 3000 CONTINUE
...
!  CORRECT ROTATIONAL DERIVATIVES
      CALL LOADAX(XR(4,2,1),XR(4,2,2))
      CALL FAST(MJDSEC,FSEC,AA,II,LL,NEQNA,XA,VMATX,                    &
     &          PXDTA3(1,1,IFILL,1,ISQ),XDDOTA(1,IORBAK,1,ISQ),XDDSUN,  &
!!!! &          XDDCMP,XDDOT(1,IORBAK),X,XMNRT,XR(4,2,1),XR(1,2,1),     &
     &          XDDDUM,XDDOT(1,IORBAK),X,XMNRT,XR(4,2,1),XR(1,2,1),     &
     &          XRDOT(4,IORBAK,1,ISQ),XRDOT(1,IORBAK,1,ISQ),.TRUE.,     &
     &          XDDTMC,.FALSE.,INTCRD,ONE,                              &
!... Begin TJS...
     &          XR(13,2,1),XRDOT(13,IORBAK,1,ISQ),XR(157,2,1),          &
     &          XRDOT(157,IORBAK,1,ISQ),XDDP,1,TRQP,PXDDT3(1,1,IFILL),  &
     &          NEQN)
!... End TJS...
!
!
!
     ...
     &          PXDTA3(1,1,IFILL,1,ISQ),XDDOTA(1,IORBAK,1,ISQ),XDDSUN,  &
!!!! &          XDDCMP,XDDOT(1,IORBAK),X,XMNRT,XR(4,2,1),XR(1,2,1),     &
     &          XDDDUM,XDDOT(1,IORBAK),X,XMNRT,XR(4,2,NAST),            &
     &          XR(1,2,NAST),                                           &
     &          XRDOT(4,IORBAK,NAST,ISQ),XRDOT(1,IORBAK,NAST,ISQ),      &
     &          .TRUE.,XDDTMC,.FALSE.,INTCRD,ONE,                       &
!... Begin TJS...
     &          XR(13,2,NAST),XRDOT(13,IORBAK,NAST,ISQ),XR(157,2,NAST), &
     &          XRDOT(157,IORBAK,NAST,ISQ),XDDP,2,TRQS,                 &
     &          PXDDT3(1,1,IFILL),NEQN)
!... End TJS...
            ENDIF
!
...
!      XGMR3S=GMS/RSQA
      XGMR3S=(GMS+GM)/RSQA
      XGMR3S=XGMR3S/R
      XDDOTA(IO+J,IORBAK,1,ISQ)=-XA(IO+J,1,1)*XGMR3S+XDDCMP(IO+J)
!
...
!
      ...
      XGMR3S=(GM2+GM)/RSQB
      XGMR3S=XGMR3S/R
      XDDOTA(IO+J,IORBAK,NAST,ISQ)=-XA(IO+J,1,NAST)*XGMR3S+XDDCM2(IO+J)
!
 3100 END DO
 3150 END DO
      LSVSTP=.FALSE.
...
! UPDATE SUMS
     ...
     &                +SUMX(J,1,IBACK)
      SUMXA(J,2,IBACK1,1,ISQ)=SUMXA(J,2,IBACK,1,ISQ)                    &
     &                +XDDOTA(J,IORBAK,1,ISQ)
      SUMXA(J,1,IBACK1,1,ISQ)=SUMXA(J,2,IBACK1,1,ISQ)                   &
     &                +SUMXA(J,1,IBACK,1,ISQ)
 3200 END DO
      IF(NAST.EQ.2) THEN
        DO 3201 J=1,N3
        SUMXA(J,2,IBACK1,2,ISQ)=SUMXA(J,2,IBACK,2,ISQ)                  &
     &                +XDDOTA(J,IORBAK,2,ISQ)
        SUMXA(J,1,IBACK1,2,ISQ)=SUMXA(J,2,IBACK1,2,ISQ)                 &
     &                  +SUMXA(J,1,IBACK,2,ISQ)
...
!...  DO 3205 J=1,12
        DO 3205 J=1,NDPOR
!... End TJS...
        SUMXR(J,2,IBACK1,1,ISQ)=SUMXR(J,2,IBACK,1,ISQ)                  &
     &                +XRDOT(J,IORBAK,1,ISQ)
        SUMXR(J,1,IBACK1,1,ISQ)=SUMXR(J,2,IBACK1,1,ISQ)                 &
     &                +SUMXR(J,1,IBACK,1,ISQ)
 3205   END DO
        IF(NAST.EQ.2) THEN
          DO 3206 J=1,NDPOR
          SUMXR(J,2,IBACK1,NAST,ISQ)=SUMXR(J,2,IBACK,NAST,ISQ)          &
     &                +XRDOT(J,IORBAK,NAST,ISQ)
          SUMXR(J,1,IBACK1,NAST,ISQ)=SUMXR(J,2,IBACK1,NAST,ISQ)         &
     &                +SUMXR(J,1,IBACK,NAST,ISQ)
 3206     END DO
        ENDIF
      ENDIF
...
!  THAT CASE IORDER=IORDRV AND H=HV
     ...
     &       PXDDAC(1,1,IFILL),NEQNAC,IPTFBS,NPRMAC,.FALSE.,.TRUE.,ONE, &
     &       DYNTIM,NPERDY,LACCELS,EXTACC,EXACIN,EXACOB,ACCTIM,ACCPER,  &
     &       NPERAC,II(KIACCP),AA(KPACCL),NRAT,XP,1)
      CALL F123(MJDST,FSECT,X,XP,PXDDT3(1,1,IFILL),NSAT,                &
     &       NEQN,NDCRD2,XP,TOPXAT(1,IFILL),LTPXAT(1,IFILL),AA,II,LL,   &
     &       ISATID,                                                    &
     &       LFHIRT,LSURFF,XNC,XPNC,                                    &
     &       PXDDAC(1,1,IFILL),NEQNAC,IPTFBS,NPRMAC,.FALSE.,.TRUE.,ONE, &
     &       DYNTIM,NPERDY,LACCELS,EXTACC,EXACIN,EXACOB,ACCTIM,ACCPER,  &
     &       NPERAC,II(KIACCP),AA(KPACCL),NRAT,XP,2)
      CALL F123(MJDST,FSECT,X,XP,PXDDT3(1,1,IFILL),NSAT,                &
     &       NEQN,NDCRD2,XP,TOPXAT(1,IFILL),LTPXAT(1,IFILL),AA,II,LL,   &
     &       ISATID,                                                    &
     &       LFHIRT,LSURFF,XNC,XPNC,                                    &
     &       PXDDAC(1,1,IFILL),NEQNAC,IPTFBS,NPRMAC,.FALSE.,.TRUE.,ONE, &
     &       DYNTIM,NPERDY,LACCELS,EXTACC,EXACIN,EXACOB,ACCTIM,ACCPER,  &
     &       NPERAC,II(KIACCP),AA(KPACCL),NRAT,XP,3)
      HQ=H
     ...
     &       PXDDAC(1,1,IFILL),NEQNAC,IPTFBS,NPRMAC,.FALSE.,.TRUE.,ONE, &
     &       DYNTIM,NPERDY,LACCELS,EXTACC,EXACIN,EXACOB,ACCTIM,ACCPER,  &
     &       NPERAC,II(KIACCP),AA(KPACCL),NRAT,XP,XDDP,XA(1,1,NAST),LQ1,&
     &       IPSBRN,LQ2,XDDBRN,HQ)
 3500 CONTINUE
...
! EVALUATE FORCE MODEL PARTIAL DERIVATIVES
     ...
     &   .FALSE.,LNDRAG)
      CALL VEVALA(PXA,PXDTA3(1,1,IOVBAK-1,1,ISQ),NEQNA,.FALSE.,VMATA,   &
     &            NEQN,PXDDT3(1,1,IOVBAK-1),.FALSE.,1)
!
!
         IF(NAST.EQ.2) THEN
      CALL VEVALA(PXA(1,1,NAST),PXDTA3(1,1,IOVBAK-1,NAST,ISQ),NEQNA,    &
     &            .FALSE.,VMATA(1,1,NAST),NEQN,PXDDT3(1,1,IOVBAK-1),    &
     &            .FALSE.,2)
         ENDIF
!
!
...
!
 ...
 3600 END DO
      DO 3601 N=1,NEQNA3
      PXA(N,1,1)=SUMPXA(N,1,IBAKV1,1,ISQ)
      PXA(N,1,NAST)=SUMPXA(N,1,IBAKV1,NAST,ISQ)
 3601 END DO
!
!
      DO 3800 I=1,IOVL2
...
!
 ...
 3700 END DO
      DO 3701 N=1,NEQNA3
      PXA(N,1,1)=PXA(N,1,1)+CCPV(I)*PXDDTA(N,K,1,ISQ)
 3701 END DO
      IF(NAST.EQ.2) THEN
        DO 3702 N=1,NEQNA3
        PXA(N,1,NAST)=PXA(N,1,NAST)+CCPV(I)*PXDDTA(N,K,NAST,ISQ)
 3702   END DO
      ENDIF
!
!
...
!
 ...
 4000 END DO
      DO 4001 N=1,NEQNA3
      PXA(N,2,1)=SUMPXA(N,2,IBAKV1,1,ISQ)
      PXA(N,2,NAST)=SUMPXA(N,2,IBAKV1,NAST,ISQ)
 4001 END DO
!
!
      DO 4200 I=1,IOVL1
...
!
 ...
 4100 END DO
      DO 4101 N=1,NEQNA3
      PXA(N,2,1)=PXA(N,2,1)+CCVV(I)*PXDDTA(N,K,1,ISQ)
 4101 END DO
      IF(NAST.EQ.2) THEN
        DO 4102 N=1,NEQNA3
        PXA(N,2,NAST)=PXA(N,2,NAST)+CCVV(I)*PXDDTA(N,K,NAST,ISQ)
 4102   END DO
      ENDIF
!
!
...
!
      DO 5202 IAST=1,NAST
!
! COMPUTE S-MATRIX FOR ASTEROID
      IF(LNDRAG)GO TO 4601
      DO 4501 J=1,3
      J1=J+3
      DO 4401 I=1,3
      I1=I+3
      SMAT(I,J)=H2C*VMATA(I,J,IAST)
      SMAT(I,J1)=H2C*VMATA(I,J1,IAST)
      SMAT(I1,J)=H1C*VMATA(I,J,IAST)
      SMAT(I1,J1)=H1C*VMATA(I,J1,IAST)
 4401 END DO
      SMAT(J,J)=SMAT(J,J)+ONE
      SMAT(J1,J1)=SMAT(J1,J1)+ONE
 4501 END DO
 ...
 4901 CONTINUE
! INVERT S-MATRIX
      CALL DNVERT(NPV3,SMAT,6,XPA)
! MULTIPLY V-MATRIX TIMES S-MATRIX INVERSE
      DO 5101 I=1,3
      DO 5101 K=1,NPV3
      SUM=ZERO
      DO 5001 J=1,NPV3
      SUM=SUM+VMATA(I,J,IAST)*SMAT(J,K)
 5001 END DO
      VMATS(K,1,I)=SUM
 5101 CONTINUE
! CHAIN FORCE MODEL PARTIALS TO EVALUATE AST VARIATIONAL EQUATIONS.
!     IKO=(ISAT-1)*3
      IKO=0
      DO 5401 J=1,NPV
      DO 5201 N=1,NEQNA
      PXDTA3(N,1+IKO,IOVBAK-1,IAST,ISQ)=                                &
     &PXDTA3(N,1+IKO,IOVBAK-1,IAST,ISQ)                                 &
     &  +PXA3(N,1+IKO,J,IAST)*VMATS(1,J,1)                              &
     &  +PXA3(N,2+IKO,J,IAST)*VMATS(2,J,1)                              &
     &  +PXA3(N,3+IKO,J,IAST)*VMATS(3,J,1)
      PXDTA3(N,2+IKO,IOVBAK-1,IAST,ISQ)=                                &
     &PXDTA3(N,2+IKO,IOVBAK-1,IAST,ISQ)                                 &
     &  +PXA3(N,1+IKO,J,IAST)*VMATS(1,J,2)                              &
     &  +PXA3(N,2+IKO,J,IAST)*VMATS(2,J,2)                              &
     &  +PXA3(N,3+IKO,J,IAST)*VMATS(3,J,2)
      PXDTA3(N,3+IKO,IOVBAK-1,IAST,ISQ)=                                &
      PXDTA3(N,3+IKO,IOVBAK-1,IAST,ISQ)                                 &
     &  +PXA3(N,1+IKO,J,IAST)*VMATS(1,J,3)                              &
     &  +PXA3(N,2+IKO,J,IAST)*VMATS(2,J,3)                              &
     &  +PXA3(N,3+IKO,J,IAST)*VMATS(3,J,3)
 5201 END DO
!
 5401 END DO
!
...
!
 ...
 5600 END DO
      DO 5601 N=1,NEQNA3
      SUMPXA(N,2,IBAKV1,1,ISQ)=SUMPXA(N,2,IBACKV,1,ISQ)                 &
     &                 +PXDDTA(N,IOVBAK,1,ISQ)
      SUMPXA(N,1,IBAKV1,1,ISQ)=SUMPXA(N,2,IBAKV1,1,ISQ)                 &
     &                 +SUMPXA(N,1,IBACKV,1,ISQ)
 5601 END DO
      IF(NAST.EQ.2) THEN
        DO 5602 N=1,NEQNA3
        SUMPXA(N,2,IBAKV1,NAST,ISQ)=SUMPXA(N,2,IBACKV,NAST,ISQ)         &
     &                   +PXDDTA(N,IOVBAK,NAST,ISQ)
        SUMPXA(N,1,IBAKV1,NAST,ISQ)=SUMPXA(N,2,IBAKV1,NAST,ISQ)         &
     &                   +SUMPXA(N,1,IBACKV,NAST,ISQ)
 5602   END DO
      ENDIF
!
!
