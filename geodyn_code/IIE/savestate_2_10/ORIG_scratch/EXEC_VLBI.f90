      SUBROUTINE EXEC_VLBI( SOU_NAM, STA1_NAM, STA2_NAM, MJD, TAI,      &
     &                    TAU_GR, TAU_PH, RATE_PH, ACCL_PH,             &
     &                    DER_DEL, DER_RAT, S_CRS, NQUABL ,C_STA,NSTA_L,&
...
! ************************************************************************
! *                                                                      *
! *   Routine EXEC_VLBI is the main routine for computation              *
...
! *                                                                      *
! *   TAU_GR ( REAL*8    ) -- Group delay. Units: seconds.               *
! *   TAU_PH ( REAL*8    ) -- Phase delay. Units: seconds.               *
...
! ************************************************************************
      ...
      INTEGER  NDER
      PARAMETER ( NDER = 17 )
      INTEGER  MJD, NQUABL,IVRB
      INTEGER  CALCTRP,ITRPXM
      DOUBLE PRECISION TAI, TAU_GR, TAU_PH, RATE_PH, ACCL_PH, &
     ...
     &           EAST_CRS(3), E_PROJ, NORTH_UEN(3),NORTH_TRS(3)
      ...
      CHARACTER  C_STA(NSTA_L)*8
      CHARACTER  C_SOU(NQUABL)*8
...
!
 ...
 210     FORMAT ( 3('TRS_TO_CRS = ', 3(F15.12,2X)/) )
      ...
      ISOU    = SOU_INDEX (  SOU_NAM,C_SOU,NQUABL)
!
...
!
     ...
     &                        AXOF_UP(J3), MOUNT_TYPE,S_CRS,NQUABL,&
...
!
     ...
     &                       TAU_GEOM, RATE_GEOM, ACCL_GEOM, &
     &                       DER_DEL(DER_EOP(1)),  &
     &                       DER_DEL(DER_STA1(1)), &
     &                       DER_DEL(DER_STA2(1)), &
     &                       DER_DEL(DER_RA),      &
     &                       DER_DEL(DER_DL), S_CRS,NQUABL,COO_TRS,  &
     &                       VEL_CRS,COO_CRS,PLAN,IER )
!     write(6,*)'  TAU_GEOM ', TAU_GEOM
!     write(6,*)'  RATE_GEOM ', RATE_GEOM
!     write(6,*)'  ACCL_GEOM ', ACCL_GEOM
...
!     write(6,*)' DER_DEL(DER_DL) ',DER_DEL(DER_DL)
     ...
     &                       DER_DEL(DER_STA2(1)), &
     &                       DER_DEL(DER_RA),      &
     &                       DER_DEL(DER_DL), S_CRS,NQUABL,COO_CRS,  &
...
!
     ...
     &              TROP_GEOM_TAU,S_CRS,NQUABL,PLAN )
         ELSE
           TROP_GEOM_TAU = 0.0D0
      END IF
!
