!$PLANPC
      SUBROUTINE PLANPC(MJDSEC,FSEC,LVELP,LBARYC,II,CBODY,TBODY)
!********1*********2*********3*********4*********5*********6*********7**
! PLANPC           92/07/17            9203.0    LUCIA TSAOUSSI
!
...
!********1*********2*********3*********4*********5*********6*********7**
     ...
     &              IPLNZ(999),NXPLNI
      ...
      DIMENSION CBODY(6),TBODY(6),SUNRF(6),BDSTAT(7,21)
      DIMENSION IMAP(11)
      DIMENSION II(1)
...
!   FIGURE POINTER WHITHIN 32 DAY RECORD FOR EACH PERIOD GROUP
      ...
      FSECC=FSCEPC(1+ITOT)+IGRPPT(I)*FNSEC(I)
  ...
  100 END DO
      ...
      BDSTAT(1,I)=BDSTAT(1,I)+EPHBFC(IBK11)*CHB(IBK2)
      BDSTAT(2,I)=BDSTAT(2,I)+EPHBFC(IBK12)*CHB(IBK2)
      BDSTAT(3,I)=BDSTAT(3,I)+EPHBFC(IBK13)*CHB(IBK2)
  120 END DO
      ...
      IBK12=(IBPT+IB)+IBK2-1
      IBK13=(IBPT+2*IB)+IBK2-1
      BDSTAT(4,I)=BDSTAT(4,I)+EPHBFC(IBK11)*CHBV(IBK2)
      BDSTAT(5,I)=BDSTAT(5,I)+EPHBFC(IBK12)*CHBV(IBK2)
      BDSTAT(6,I)=BDSTAT(6,I)+EPHBFC(IBK13)*CHBV(IBK2)
  140 END DO
...
!
       IF(LXEPHM) THEN
       CALL PLNPCX(MJDSEC,FSEC,BDSTAT(1,12+IASTSN),II(KISUPE),LVELP)
       ENDIF
!      IF(LXEPHM) THEN
!      CALL PLNPCX(MJDSEC,FSEC,XSTAT,II(KISUPE),LVELP)
!      ENDIF
      NUL=3
      IF(LVELP) NUL=6
!
