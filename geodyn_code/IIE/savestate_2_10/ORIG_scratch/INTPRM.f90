!$INTPRM
      SUBROUTINE INTPRM(XSTT1,XSTT2,XSTR1,XSTR2,XS1,XS2,                &
     &                  COEFEM,COEFMO,COEFM,                            &
     &                  IGRPX,IORCBX,IORTBX,IORMOX,COUNT,LTWO,NM,       &
     &                  RANGET,RANGER,RANGE2,RANGE4,RRT,RRR,            &
     &                  XHEBS1,XHEBS2,CHEBT1,CHEBT2,CHEBR1,             &
...
!********1*********2*********3*********4*********5*********6*********7**
     ...
     &          RANGET(NM),RANGER(NM),RRT(NM),RRR(NM)
      DIMENSION XHEBS1(NM,IORCB),XHEBS2(NM,IORCB),                      &
     ...
     &          RANGE2(NM),RANGE4(NM)
      DIMENSION BSC(15),BSC2(15),DELTS2(1000)
      DIMENSION CHEBS1(1000,15),CHEBS2(1000,15)
      DIMENSION CHEBS3(1000,15),CHEBS4(1000,15)
...
! COMPUTE TIME DIFFERENCES
   ...
   80 END DO
      ...
      DELTS(I)=(AA(JFSCM2+I-1)-AA(JFSCM1+I-1))*VSEC4
      DELTS2(I)=(AA(JFSCM2+I-1)-AA(JFSCM1+I-1))*VSEC2
...
!
      DO 95 I=1,NM
!
      DTQ=CBR1MO(I,2)+(AA(JFSCM1-1+I)-AA(JFSCN1-1+I))*VSEC4
      BSC(1)=1.D0
      CALL CHEBPL(BSC,BSC2,DTQ,IORMO,.FALSE.)
      DO J=1,IORMO
        CHEBS1(I,J)=BSC(J)
      ENDDO
!
      DTQ=CBR2MO(I,2)+(AA(JFSCM2-1+I)-AA(JFSCN2-1+I))*VSEC4
      BSC(1)=1.D0
      CALL CHEBPL(BSC,BSC2,DTQ,IORMO,.FALSE.)
      DO J=1,IORMO
        CHEBS2(I,J)=BSC(J)
      ENDDO
!
      DTQ=CHEBR1(I,2)+(AA(JFSCM1-1+I)-AA(JFSCN1-1+I))*VSEC2
      BSC(1)=1.D0
      CALL CHEBPL(BSC,BSC2,DTQ,IORTB,.FALSE.)
      DO J=1,IORTB
        CHEBS3(I,J)=BSC(J)
      ENDDO
!
      DTQ=CHEBR2(I,2)+(AA(JFSCM2-1+I)-AA(JFSCN2-1+I))*VSEC2
      BSC(1)=1.D0
      CALL CHEBPL(BSC,BSC2,DTQ,IORTB,.FALSE.)
      DO J=1,IORTB
        CHEBS4(I,J)=BSC(J)
...
!
      DO J=1,IORMO
        ...
        BSC2(J)=CHEBS2(I,J)
      ENDDO
      CALL CHEBDF( BSC,BSC2,DELTS(I),1,IORMO)
      DO J=1,IORMO
        CHEBS1(I,J)=BSC(J)
        CHEBS2(I,J)=BSC2(J)
      ENDDO
!
      DO J=1,IORTB
        ...
        BSC2(J)=CHEBS4(I,J)
      ENDDO
      CALL CHEBDF( BSC,BSC2,DELTS2(I),1,IORTB)
      DO J=1,IORTB
        CHEBS3(I,J)=BSC(J)
        CHEBS4(I,J)=BSC2(J)
      ENDDO
...
!
      ...
      JJ=IORMO+2-J
      DO 390 K=1,NM
      DXM(K,I) = DXM(K,I)+EMFCT2*COEFMO(I,JJ)*CHEBS2(K,JJ)
  390 END DO
  400 END DO
!
      DO 420 J=2,IORTB
      JJ=IORTB+2-J
      DO 410 K=1,NM
      DXM(K,I) = DXM(K,I)+COEFEM(I,JJ)*CHEBS4(K,JJ)
  410 END DO
  420 END DO
!
!**********DEBUG*********
