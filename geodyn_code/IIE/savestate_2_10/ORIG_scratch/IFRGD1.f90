!$IFRGD1
      SUBROUTINE IFRGD1(AA,II,LL,IFFHDR,IFFLEN,FFBUF)
!********1*********2*********3*********4*********5*********6*********7**
! IFRGD1           00/00/00            0000.0    PGMR - B. EDDY
!
...
! KACOEF
      ...
      IWORDS=IWORDS+NX
      CALL BINRD2(IUNT11,FFBUF,NX  )
      IF(NX.GT.0)  CALL Q9ICLA(FFBUF,AA(KACOEF),NX,ISTAT)
! KACTIM
      IREC=IREC+1
      NX=IFFLEN(IREC)
      IF(NX.GT.IFBUFR) GO TO 60100
      IWORDS=IWORDS+NX
      CALL BINRD2(IUNT11,FFBUF,NX  )
      IF(NX.GT.0)  CALL Q9ICLA(FFBUF,AA(KACTIM),NX,ISTAT)
! KPPER/PPER
      IREC=IREC+1
      NX=IFFLEN(IREC)
      IF(NX.GT.IFBUFR) GO TO 60100
      IWORDS=IWORDS+NX
      CALL BINRD2(IUNT11,FFBUF,NX  )
      IF(NX.GT.0)  CALL Q9ICLA(FFBUF,AA(KPPER),NX,ISTAT)
!
...
! KISNT
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KISNT),IFFLEN(IREC) )
! KMM-KQQ
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KMM),IFFLEN(IREC) )
! KIGTSR --- TIME DEPENDENT GRAVITY
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KIGTSR),IFFLEN(IREC) )
! KICNTT --- POINTER TO ADJUSTED C IN THE TIME PERIOD C COEFFICIENT ARRA
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KICNTT),IFFLEN(IREC) )
! KISNTT --- POINTER TO ADJUSTED S IN THE TIME PERIOD S COEFFICIENT ARRA
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KISNTT),IFFLEN(IREC) )
! KTPGRC --- TIMES FOR TIME PERIOD GRAVITY - C COEFF
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KTPGRC),IFFLEN(IREC) )
! KTPGRS --- TIMES FOR TIME PERIOD GRAVITY - S COEFF
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KTPGRS),IFFLEN(IREC) )
! KTPC --- POINTERS TO TIME ARRAYS FOR TIME PERIOD GRAVITY - C COEFF
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KTPC),IFFLEN(IREC) )
! KTPS --- POINTERS TO TIME ARRAYS FOR TIME PERIOD GRAVITY - S COEFF
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KTPS),IFFLEN(IREC) )
! KICNTP --- POINTERS TO ICNTP (ICNT) ARRAYS FOR TIME PERIOD GRAVITY
!            C COEFFICIENT
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KICNTP),IFFLEN(IREC) )
! KISNTP --- POINTERS TO ISNTP (ISNT) ARRAYS FOR TIME PERIOD GRAVITY
!            C COEFFICIENT
      IREC=IREC+1
      IWORDS=IWORDS+IFFLEN(IREC)
      CALL BINRD(IUNT11,II(KISNTP),IFFLEN(IREC) )
!
! TEST IF CORRECT NUMBER OF DATA RECORDS READ
!
      IF(IREC.NE.NRECS) GO TO 60200
...
89999 FORMAT(1X,'EXECUTION TERMINATING IN SUBROUTINE IFRGD1 ')
      END
