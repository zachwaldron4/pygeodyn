!$SARLAT
      SUBROUTINE SARLAT(MJDS,FSEC,XSAT,VSAT,BFNRM1,BFNRM2,BFNRM3,       &
     &                  TDNRM1,TDNRM2,TDNRM3,CTHETA,NFACE,NMOVE,LFORCE, &
     &                  IDSATS,IDATTB,SABIAS,TIMBI1,TIMBI2,VLOUVS,      &
     &                  NSTLOV,ISLVID,TSLOUV,AA,ISATID,TOTROT,ATROT)
!*******************************************************************
!  ROUTINE NAME:   SARLAT   DATE: 11/04/13      PGMR: J. WIMERT
!
!   FUNCTION - TO COMPUTE ROTATION FROM SARAL BODY-FIXED FRAME TO THE
!              GEODYN TRUE OF REFERENCE FRAME, BASED ON THE SARAL
...
!
!************ SARAL LOCAL VARIABLE DEFFINITIONS************************
...
!   YTOD     A      W    TOD VECTOR TANGENT TO ORBIT PLANE
!   ZTOD     A      W    TOD VECTOR NORMAL TO ORBIT PLANE
!   XTOD     A      W    TOD VECTOR FROM SPACECRAFT TO EARTH CENTER
...
!
! NOTES:
!    MODIFIED FROM HY2A ATTITUDE SUBROUTINE (HY2ATT)
!
...
!         XTOD = TOD VECTOR FROM S/C TO CENTER OF EARTH
!         ZTOD = TOD VECTOR NORMAL TO  ORBIT PLANE (Z = X X VSAT)
!         YTOD = TOD VECTOR TANGENT TO ORBIT PLANE (Y = Z X X)
!
! XAXIS ROTATION
! radial
! negative because xsat is pointing from earth

      XMAG = SQRT(XSAT(1)**2+XSAT(2)**2+XSAT(3)**2)
      XTOD(1) =-XSAT(1)/XMAG
      XTOD(2) =-XSAT(2)/XMAG
      XTOD(3) =-XSAT(3)/XMAG
      SPF(1,1)= XTOD(1)
...
! ZAXIS ROTATION
! cross
!  dont take negative, look atfigure in documentation
!  follow right hand rule for cross products

      ZTOD(1) = ( XTOD(2)*VSAT(3)-XTOD(3)*VSAT(2))
      ZTOD(2) = ( XTOD(3)*VSAT(1)-XTOD(1)*VSAT(3))
      ZTOD(3) = ( XTOD(1)*VSAT(2)-XTOD(2)*VSAT(1))
...
!
!
! COMPUTE UNIT VECTOR DEFINING ZERO ORBIT ANGLE (ZTOD X SUN)
! use ZTOD because it is cross track?
!
      VORB0(1) = ZTOD(2)*BDTRUE(3,8)-ZTOD(3)*BDTRUE(2,8)
      VORB0(2) = ZTOD(3)*BDTRUE(1,8)-ZTOD(1)*BDTRUE(3,8)
      VORB0(3) = ZTOD(1)*BDTRUE(2,8)-ZTOD(2)*BDTRUE(1,8)
...
! NO MOVEABLE PLATES, SKIP SOLAR ARRAY ROTATION
!
...
! IF MOVEABLE PLATES EXIST COMPUTE THEIR ROTATIONS
      ...
      WRITE(6,*) 'STOP IN SARLAT.  NO MOVEABLE PLATES ANTCIPATED'
      STOP
  850 CONTINUE
...
! ROTATE BODY-FIXED LOUVER ACCELERATION VECTOR IF NECESSARY
 ...
 !
! OUTPUT SARAL TELEM FILE INFORMATION
...
!....PERTINENT ANGLES
     ...
     &             CM999,CM999,CM999,QAT(1),QAT(2),QAT(3), &
