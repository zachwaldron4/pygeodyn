...
!
         if(kentry.eq.1)then
             WRITE(6,*) 'CHECK -- DRAG.f90:input 86 but return MSIS2.0'
         endif
         IMARK = IND3HR
         COSHLN = -COSHL
         SINHLN = -SINHL
         ...
         C(1)=DRHODZ
          if(kentry.eq.1)then
              WRITE(6,*) 'CHECK -- DRAG.f90: Called MSIS.f90 with MSIS2'
          endif
!
      ENDIF
!      IF(IATDN.EQ.6) THEN
...
!         IYR=(IYMD/10000.D0)+0.5D0
!         CALL MSIS(DAY,IYR,ALTI,PHI,XLAMB,COSHLN,SINHLN,FLUXIN,FLXAVG,  &
!     &          FLUXM(1),IMARK,IKPAP,I324,IDRV,RHO,DRHODZ,IERR,MSIS_FLAG)
!         C(1)=DRHODZ
!          if(kentry.eq.1)then
!              WRITE(6,*) 'CHECK DRAG.f90: DENSITY MODEL IS msis2'
!          endif
!!
!      ENDIF
!
