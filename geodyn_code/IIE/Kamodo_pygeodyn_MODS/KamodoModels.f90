!$KAMODO-ctipe
      SUBROUTINE KamodoModels(MJDSEC,FSEC,ALTM,XLATD,XLOND,        &
...
!       &                                            G,H,II,J,K
        ...
        !! Use internal file to write CHAR to DP_float
!        read( message, '(D25.20)' ) rho1    ! reads the value from string and assigns it to float
...
!                                                                       &
        ...
        call date_and_time(VALUES=values)
        !print '(8i5)', values!
        end_t_ms = values(7)*1000+values(8)
        time_elapsed_ms = (real(end_t_ms) - real(start_t_ms))*0.001
        WRITE(6,*) 'time_elapsed_ms:             ', time_elapsed_ms
