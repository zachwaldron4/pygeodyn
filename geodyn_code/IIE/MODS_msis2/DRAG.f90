...
!     
      if(kentry.eq.1)then  
          WRITE(6,*) 'CHECK -- USING MODS_msis2/DRAG.f90'
          WRITE(6,*) 'CHECK -- IN Drag.f90 IATDEN = ', IATDEN
          WRITE(6,*) 'CHECK -- IN Drag.f90 IATDN = ', IATDN
          FLUSH(6)
      endif
!      
!      FLUSH(6)
...
!
      ...
      IF(IATDN.EQ.5) THEN
!
!        ....CALL TO MSIS86 INSTEAD OF DTM
!
         if(kentry.eq.1)then
              WRITE(6,*) 'CHECK -- DRAG.f90:input 86 but return MSIS2'
...
!*  PRINTOUT FOR TESTING.
!         PRINT 801, DAY,IYR,ALTI,PHI,XLAMB,COSHLN,SINHLN,FLUXIN,FLXAVG, &
!                & FLUXM(1),IMARK,IKPAP,I324,IDRV,RHO,DRHODZ,IERR
!
!  801 FORMAT(1X,  '    ',/,                                             &
!              1X, '     INPUT to MSIS.f90 from DRAG.f90:  ',/,          &
...
!              1X, '     ' )
     ...
     &          FLUXM(1),IMARK,IKPAP,I324,IDRV,RHO,DRHODZ,IERR)
         C(1)=DRHODZ
          if(kentry.eq.1)then
              WRITE(6,*) 'CHECK -- DRAG.f90:Called MSIS.f90 w MSIS2'
          endif
!
      ENDIF
!
...
!
  160 CONTINUE 
      ...
      if(kentry.eq.1)then
          WRITE(6,*) 'CHECK -- DRAG.f90-writing density file to 99'
          write(99,'(a12,2a8,2a12,a12,2a20,3a15,3a15)') &
