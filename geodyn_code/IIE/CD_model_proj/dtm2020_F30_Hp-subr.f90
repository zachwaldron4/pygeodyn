...
subroutine dtm5(day,f,fbar,ap60,alti,hl,alat,xlon,tz,tinf,ro,d,wmm)
  ...
  real, dimension(10), intent(in) :: ap60
...
& status='old')  
  ...
  !
  do i=1,npdtm
    read(146,540,end=20) ni,tt(i),dtt,h(i),dh,he(i),dhe,o(i),dox, &
            az2(i),daz2,o2(i),do2,az(i),daz,t0(i),dt0,tp(i),dtp
  end do
!
20 continue
...
!
  ...
  !
  pi=acos(-1.)
  if(XLONG.EQ.291.0000) XLONG=291.1
  RFAC=PI/180.
  PLATR=78.5*RFAC
  PLONGR=291.*RFAC
  SPL=SIN(PLATR)
